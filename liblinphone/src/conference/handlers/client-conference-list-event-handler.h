...
public:
	...
	bool handlesEvent(const std::shared_ptr<Event> &event) const;
...
private:
	...
	std::optional<std::shared_ptr<EventSubscribe>> findEvent(const std::shared_ptr<Event> &event) const;

	void deleteEvent(const std::shared_ptr<Event> &event);
	void addEvent(const std::shared_ptr<EventSubscribe> &event);
	...
	std::optional<std::shared_ptr<EventSubscribe>>
	subscribe(const std::shared_ptr<Event> &event, const std::map<std::string, std::shared_ptr<Address>> &addresses);
