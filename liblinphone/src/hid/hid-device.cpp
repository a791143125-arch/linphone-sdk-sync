...
HidDevice::HidDevice(const std::shared_ptr<Core> &core,
                     ...
                     void *device,
                     const std::shared_ptr<HidDescriptor> &descriptor)
...
std::shared_ptr<HidDevice> HidDevice::create(const std::shared_ptr<Core> &core,
	...
	const auto descriptor = std::make_shared<HidDescriptor>(buf, static_cast<size_t>(length));
	return std::make_shared<HidDevice>(core, productId, productName, serialNumber, device, descriptor);
}

void HidDevice::initializeFromDescriptor() {
	...
	for (const auto &report : reports) {
		switch (report->getReportType()) {
			case HidDescriptor::ReportType::Input:
				...
				if (reportId == mInputData.mReportId && report->getReportSize() == 1) {
					if (report->getUsagePage() == static_cast<uint32_t>(HidDescriptor::UsagePage::Telephony)) {
						size_t localOffset = 0;
						for (const auto usage : report->getUsages()) {
							if (usage == static_cast<uint32_t>(HidDescriptor::TelephonyUsage::HookSwitch)) {
								mInputData.mHookSwitch = 1 << (inputOffset + localOffset);
							} else if (usage == static_cast<uint32_t>(HidDescriptor::TelephonyUsage::Flash)) {
								mInputData.mHookFlash = 1 << (inputOffset + localOffset);
							} else if (usage == static_cast<uint32_t>(HidDescriptor::TelephonyUsage::PhoneMute)) {
								mInputData.mPhoneMute = 1 << (inputOffset + localOffset);
							}
							localOffset++;
						}
					} else if (report->getUsagePage() == static_cast<uint32_t>(HidDescriptor::UsagePage::Button)) {
						size_t localOffset = 0;
						for (const auto usage : report->getUsages()) {
							if (usage == static_cast<uint32_t>(HidDescriptor::TelephonyUsage::ProgrammableButton)) {
				...
				break;
			case HidDescriptor::ReportType::Output:
				reportId = static_cast<uint8_t>(report->getReportID());
				...
				if (reportId == mOutputData.mReportId && report->getReportSize() == 1) {
					if (report->getUsagePage() == static_cast<uint32_t>(HidDescriptor::UsagePage::LEDs)) {
						size_t localOffset = 0;
						for (const auto usage : report->getUsages()) {
							if (usage == static_cast<uint32_t>(HidDescriptor::LedsUsage::Mute)) {
								mOutputData.mMute = 1 << (outputOffset + localOffset);
							} else if (usage == static_cast<uint32_t>(HidDescriptor::LedsUsage::OffHook)) {
								mOutputData.mOffHook = 1 << (outputOffset + localOffset);
							} else if (usage == static_cast<uint32_t>(HidDescriptor::LedsUsage::Ring)) {
								mOutputData.mRinger = 1 << (outputOffset + localOffset);
							} else if (usage == static_cast<uint32_t>(HidDescriptor::LedsUsage::Hold)) {
				...
				break;
			case HidDescriptor::ReportType::Feature:
				// Not handled
				break;
		}
	}
