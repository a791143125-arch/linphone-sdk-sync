...
HidDescriptor::HidDescriptor(const uint8_t *data, const size_t length)
...
std::vector<std::shared_ptr<const HidDescriptor::Report>> HidDescriptor::getReports() const {
	return mApplicationCollection->getReports();
}

std::string HidDescriptor::getRawData() const {
...
void HidDescriptor::setParsingState(const ParsingState parsingState) {
...
[[nodiscard]] bool HidDescriptor::isUsagePage(UsagePage usagePage) const {
	return static_cast<uint32_t>(usagePage) == mGlobalState.usagePage;
}

[[nodiscard]] bool HidDescriptor::hasTelephonyUsage(TelephonyUsage usage) const {
...
void HidDescriptor::updateGlobalState(const uint32_t tag, const uint32_t unsignedValue, const int32_t signedValue) {
...
void HidDescriptor::updateLocalState(const uint32_t tag, const uint32_t unsignedValue) {
...
void HidDescriptor::handleMain(const uint32_t tag, const uint32_t unsignedValue) {
...
void HidDescriptor::clearLocalState() {
	mLocalState = LocalState{};
}

int32_t HidDescriptor::readSigned(const uint8_t *data, const size_t size) {
	int32_t value = 0;
...
uint32_t HidDescriptor::readUnsigned(const uint8_t *data, const size_t size) {
	uint32_t value = 0;
	for (size_t i = 0; i < size; i++) {
