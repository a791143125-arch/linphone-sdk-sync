...
long long MainDbPrivate::insertChatRoom(const shared_ptr<AbstractChatRoom> &chatRoom,
	...
	if (q->isInitialized()) {
		...
		try {
			...
			const long ephemeralNotReadLifetime = chatRoom->getEphemeralNotReadLifetime();
...
long long
MainDbPrivate::selectOneOnOneChatRoomId(long long sipAddressIdA, long long sipAddressIdB, bool encrypted) const {
#ifdef HAVE_DB_STORAGE
	long long chatRoomId;
	...
	soci::session *session = dbSession.getBackendSession();
	*session << Statements::get(Statements::SelectOneOnOneChatRoomId), soci::use(sipAddressIdA, "1"),
...
shared_ptr<EventLog> MainDbPrivate::selectConferenceChatMessageEvent(const shared_ptr<AbstractChatRoom> &chatRoom,
	...
	if (!chatMessage) {
		...
		if (row.get_indicator(20) != soci::i_null) {
			dChatMessage->enableEphemeralWithTime(static_cast<long>(row.get<double>(20)),
			                                      static_cast<long>(row.get<double>(28)));
...
shared_ptr<EventLog> MainDbPrivate::selectConferenceEphemeralMessageEvent(const ConferenceId &conferenceId,
	...
	return make_shared<ConferenceEphemeralMessageEvent>(type, getConferenceEventCreationTimeFromRow(row), conferenceId,
	                                                    static_cast<long>(row.get<double>(22)),
	                                                    static_cast<long>(row.get<double>(29)));
}
...
long long MainDbPrivate::insertConferenceChatMessageEvent(const shared_ptr<EventLog> &eventLog) {
	...
	*dbSession.getBackendSession()
	...
	if (isEphemeral) {
		...
		long ephemeralNotReadLifetime = chatMessage->getEphemeralNotReadLifetime();
		...
		*dbSession.getBackendSession() << "INSERT INTO chat_message_ephemeral_event ("
		                                  "  event_id, ephemeral_lifetime, ephemeral_not_read_lifetime, expired_time"
		                                  ") VALUES ("
		                                  "  :eventId, :ephemeralLifetime, :ephemeralNotReadLifetime, :expireTime"
		                                  ")",
		    soci::use(eventId), soci::use(ephemeralLifetime), soci::use(ephemeralNotReadLifetime),
		    soci::use(expireTime.first);
	}
...
long long MainDbPrivate::insertConferenceEphemeralMessageEvent(const shared_ptr<EventLog> &eventLog) {
	...
	long notReadLifetime =
	    static_pointer_cast<ConferenceEphemeralMessageEvent>(eventLog)->getEphemeralMessageNotReadLifetime();

	soci::session *session = dbSession.getBackendSession();
	*session << "INSERT INTO conference_ephemeral_message_event (event_id, lifetime, not_read_lifetime)"
	            " VALUES (:eventId, :lifetime, :notReadLifetime)",
	    soci::use(eventId), soci::use(lifetime), soci::use(notReadLifetime);
...
std::shared_ptr<Address> MainDb::findMissingOneOnOneConferenceChatRoomParticipantAddress(
...
std::shared_ptr<Address> MainDb::findOneOnOneConferenceChatRoomAddress(const std::shared_ptr<Address> &participantA,
	...
	return L_DB_TRANSACTION {
		...
		const long long &chatRoomId =
		    d->selectOneOnOneChatRoomId(participantASipAddressId, participantBSipAddressId, encrypted);
...
void MainDb::insertOneOnOneConferenceChatRoom(const shared_ptr<AbstractChatRoom> &chatRoom, bool encrypted) {
#ifdef HAVE_DB_STORAGE
	L_ASSERT(linphone_core_conference_server_enabled(chatRoom->getCore()->getCCore()));
	L_ASSERT(chatRoom->getCapabilities() & ChatRoom::Capabilities::OneToOne);
	...
	L_DB_TRANSACTION {
		...
		if (chatRoomId == -1) {
			...
			*d->dbSession.getBackendSession() << Statements::get(Statements::InsertOneOnOneChatRoom, getBackend()),
