...
shared_ptr<AbstractChatRoom> Core::findChatRoom(const ConferenceId &conferenceId, bool logIfNotFound) const {
	...
	auto alreadyExhumedOneOnOne = d->findExhumedChatRoomFromPreviousConferenceId(conferenceId);
	if (alreadyExhumedOneOnOne) {
		lWarning() << conferenceId << " is assigned to already exhumed " << *alreadyExhumedOneOnOne
		           << " with new conference ID " << alreadyExhumedOneOnOne->getConferenceId() << ".";
