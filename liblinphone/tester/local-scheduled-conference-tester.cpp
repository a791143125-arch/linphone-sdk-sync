...
static void conference_with_media_lost_base(bool_t server_drops) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		check_call_establishment({focus, marie, laure}, members, std::make_pair(focus.getCMgr(), focus_stat),
		                         member_stats_list, marie.getCMgr(), confAddr, security_level, encryption, FALSE, FALSE,
		                         TRUE);
...
static void conference_with_server_no_rtp_timeout() {
	...
	{ // to make sure focus is destroyed after clients.
		...
		check_call_establishment({focus, marie, laure, pauline, michelle, berthe, lise}, members,
...
static void alone_in_conference_with_chat_exits_enter() {
	...
	{ // to make sure focus is destroyed after clients.
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, {pauline.getCMgr()},
		                         std::make_pair(focus.getCMgr(), focus_stat), one_member_stats_list, marie.getCMgr(),
		                         confAddr, LinphoneConferenceSecurityLevelNone, LinphoneMediaEncryptionNone, TRUE,
		                         FALSE, TRUE);
		...
		wait_for_conference_streams({focus, marie, pauline, laure, michelle, berthe}, conferenceMgrs, focus.getCMgr(),
		                            memberList, confAddr, TRUE, LinphoneConferenceSecurityLevelNone);
...
static void conference_with_participants_late_except_one() {
	...
	{ // to make sure focus is destroyed after clients.
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, {pauline.getCMgr()},
		                         std::make_pair(focus.getCMgr(), focus_stat), one_member_stats_list, marie.getCMgr(),
		                         confAddr, LinphoneConferenceSecurityLevelNone, LinphoneMediaEncryptionNone, FALSE,
		                         FALSE, TRUE);
...
static void create_simple_conference_db_conference_scheduler_base(bool_t server_restart) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
		                         conference_address, security_level, encryption, FALSE, FALSE, FALSE);
		memberList = fill_member_list(members, participantList, marie.getCMgr(), participants_info);
		wait_for_conference_streams({focus, marie, pauline, laure, michelle, berthe}, conferenceMgrs, focus.getCMgr(),
		                            memberList, conference_address, FALSE, security_level);
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (int iteration = 0; iteration < max_iterations; iteration++) {
			...
			for (auto mgr : members) {
				...
				check_call_establishment({focus, marie, pauline}, addedMembers,
				                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list,
				                         marie.getCMgr(), confAddr, security_level, encryption, TRUE, FALSE, FALSE);
				std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
...
static void participant_joins_simple_conference_with_screen_sharing() {
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto mgr : members) {
			...
			check_call_establishment({focus, marie, pauline}, addedMembers, std::make_pair(focus.getCMgr(), focus_stat),
			                         member_stats_list, marie.getCMgr(), confAddr, security_level, encryption, FALSE,
			                         FALSE, FALSE);
			std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
			    fill_member_list(addedMembers, participantList, marie.getCMgr(), participants_info);
			wait_for_conference_streams({focus, marie, pauline}, addedConferenceMgrs, focus.getCMgr(), memberList,
...
static void create_conference_with_codec_mismatch_base(bool_t organizer_codec_mismatch) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto mgr : members) {
			auto itMgr = std::find(codec_mismatch_members.begin(), codec_mismatch_members.end(), mgr);
			if (itMgr == codec_mismatch_members.end()) {
...
static void create_conference_with_server_restart_base(bool_t organizer_first) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		std::list<std::pair<LinphoneCoreManager *, stats>> one_member_stats_list{
		    std::make_pair(first_to_join, first_to_join->stat)};
		...
		check_call_establishment({focus, marie, pauline, laure}, {first_to_join},
		                         std::make_pair(focus.getCMgr(), focus_stat), one_member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, LinphoneMediaEncryptionNone, FALSE, FALSE, TRUE);

		focus_stat = focus.getStats();
		std::list<std::pair<LinphoneCoreManager *, stats>> other_member_stats_list;
		for (auto mgr : other_members) {
			other_member_stats_list.push_back(std::make_pair(mgr, mgr->stat));
			LinphoneCallParams *new_params = linphone_core_create_call_params(mgr->lc, nullptr);
			ms_message("%s is calling conference %s", linphone_core_get_identity(mgr->lc), conference_address_str);
			linphone_core_invite_address_with_params_2(mgr->lc, confAddr, new_params, NULL, nullptr);
			linphone_call_params_unref(new_params);
		...
		check_call_establishment({focus, marie, pauline, laure}, members, std::make_pair(focus.getCMgr(), focus_stat),
		                         other_member_stats_list, marie.getCMgr(), confAddr, security_level,
		                         LinphoneMediaEncryptionNone, FALSE, FALSE, TRUE);
		std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
		    fill_member_list(members, participantList, marie.getCMgr(), participants_info);
		wait_for_conference_streams({focus, marie, pauline, laure}, conferenceMgrs, focus.getCMgr(), memberList,
...
static void create_simple_conference_with_participant_no_label_base(LinphoneConferenceLayout layout) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		check_call_establishment({focus, marie, pauline, laure, michelle}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, LinphoneMediaEncryptionNone, FALSE, FALSE, TRUE);
		std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
		    fill_member_list(members, participantList, marie.getCMgr(), participants_info);
...
static void create_conference_with_chat_client_enters_conference() {
	...
	{ // to make sure focus is destroyed after clients.
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, participants,
		                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, LinphoneMediaEncryptionNone, TRUE, FALSE, FALSE);
		std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
		...
		for (auto mgr : {marie.getCMgr(), pauline.getCMgr(), laure.getCMgr()}) {
			...
			if (pinfo) {
				...
				check_conference_info_members(pinfo, NULL, confAddr, marie.getCMgr()->identity, participants_info2,
				                              start_time, duration_m, initialSubject, description, 0,
				                              LinphoneConferenceInfoStateNew, security_level, FALSE, TRUE,
				                              account_for_conference_settings ? enable_video : TRUE,
				                              account_for_conference_settings ? enable_chat : FALSE);
				bctbx_list_free_with_data(participants_info2, (bctbx_list_free_func)linphone_participant_info_unref);
				linphone_conference_info_unref(pinfo);
			}
		}
...
static void uninvited_participant_rejoins() {
	...
	{ // to make sure focus is destroyed after clients.
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), one_member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, LinphoneMediaEncryptionNone, FALSE, FALSE, FALSE);
		memberList = fill_member_list(members, participantList, marie.getCMgr(), participants_info);
		wait_for_conference_streams({focus, marie, pauline, laure, michelle, berthe}, conferenceMgrs, focus.getCMgr(),
		                            memberList, confAddr, FALSE, security_level);
		...
		ms_message("%s rejoins conference %s", linphone_core_get_identity(berthe.getLc()),
		           Address::toCpp(confAddr)->toString().c_str());
		one_member_stats_list = {std::make_pair(berthe.getCMgr(), berthe.getStats())};
...
static void rejoining_conference_after_end(int cleanup_period, bool_t enable_chat) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto iter = 0; iter < 2; iter++) {
			...
			check_call_establishment({focus, marie, pauline, laure, michelle}, members,
			                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
			                         confAddr, security_level, LinphoneMediaEncryptionNone, enable_chat, FALSE, TRUE);
...
static void create_simple_conference_in_sfu_payload_mode() {
	...
	{ // to make sure focus is destroyed after clients.
		...
		check_call_establishment({focus, marie, pauline}, members, std::make_pair(focus.getCMgr(), focus_stat),
		                         member_stats_list, marie.getCMgr(), confAddr, LinphoneConferenceSecurityLevelNone,
		                         LinphoneMediaEncryptionNone, FALSE, FALSE, TRUE);
		std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
		    fill_member_list(members, participantList, marie.getCMgr(), participants_info);
		wait_for_conference_streams({focus, marie, pauline}, conferenceMgrs, focus.getCMgr(), memberList, confAddr,
		                            FALSE, LinphoneConferenceSecurityLevelNone);
...
static void conference_with_anonymous_participants_base(LinphoneConferenceLayout layout, bool_t enable_chat) {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		ClientConference marie("marie_rc", focus.getConferenceFactoryAddress());
		ClientConference pauline("pauline_rc", focus.getConferenceFactoryAddress());
		ClientConference laure("empty_rc", focus.getConferenceFactoryAddress());
		ClientConference michelle("michelle_rc", focus.getConferenceFactoryAddress());
		ClientConference berthe("empty_rc", focus.getConferenceFactoryAddress());
		ClientConference lise("empty_rc", focus.getConferenceFactoryAddress());
		...
		time_t start_time = ms_time(NULL);
		time_t end_time = -1;
		const char *initialSubject = "Conference with anonymous";
		...
		for (auto mgr : members) {
			...
			ms_message("%s is entering conference %s", linphone_core_get_identity(mgr->lc), conference_address_str);
			LinphoneAddress *confAddr2 = linphone_address_clone(confAddr);
			linphone_address_set_transport(confAddr2, LinphoneTransportTls);
			linphone_core_invite_address_with_params_2(mgr->lc, confAddr2, new_params, NULL, nullptr);
			linphone_address_unref(confAddr2);
			linphone_call_params_unref(new_params);
			LinphoneCall *participant_call = linphone_core_get_call_by_remote_address2(mgr->lc, confAddr);
			BC_ASSERT_PTR_NOT_NULL(participant_call);
			if (participant_call && (mgr != laure.getCMgr()) && (mgr != berthe.getCMgr())) {
				LinphoneCallLog *call_log = linphone_call_get_call_log(participant_call);
				BC_ASSERT_TRUE(linphone_call_log_was_conference(call_log));
			}
		}
		...
		LinphoneAddress *laure_identity = linphone_address_new(linphone_core_get_identity(laure.getLc()));
		participantList.insert(
		    std::make_pair(laure.getCMgr(), add_participant_info_to_list(&participants_info, laure_identity,
		                                                                 LinphoneParticipantRoleSpeaker, -1)));
		...
		for (const auto &mgr : members) {
			...
			if (participant_call) {
				...
				BC_ASSERT_EQUAL(linphone_call_params_screen_sharing_enabled(call_cparams), is_screen_sharing_enabled,
				                int, "%0d");
			}

			LinphoneCall *ccall = get_peer_call(focus.getCMgr(), participant_call);
			BC_ASSERT_PTR_NOT_NULL(ccall);
			if (ccall) {
				const LinphoneCallParams *call_cparams = linphone_call_get_current_params(ccall);
		...
		if (enable_chat) {
			...
			if (marie_conference) {
				...
				if (marie_chat_room) {
					std::string msg_text =
					    std::string("Hi everyone.\nThis is conference ") + Address::toCpp(confAddr)->toString();
					LinphoneChatMessage *msg = ClientConference::sendTextMsg(marie_chat_room, msg_text);

					BC_ASSERT_TRUE(
					    CoreManagerAssert({focus, marie, pauline, michelle, laure, berthe, lise}).wait([&msg] {
						    return (linphone_chat_message_get_state(msg) == LinphoneChatMessageStateDelivered);
					    }));
					linphone_chat_message_unref(msg);
					...
					for (ClientConference &core : cores2) {
						LinphoneAccount *account = linphone_core_get_default_account(core.getLc());
						LinphoneAddress *deviceAddress =
						    account ? linphone_account_get_contact_address(account) : nullptr;
						LinphoneChatRoom *chatRoom = core.searchChatRoom(deviceAddress, confAddr);
						BC_ASSERT_PTR_NOT_NULL(chatRoom);
						if (chatRoom) {
							BC_ASSERT_TRUE(CoreManagerAssert({focus, marie, pauline, michelle, laure, berthe, lise})
			...
			LinphoneConference *berthe_conference =
			    linphone_core_search_conference(berthe.getLc(), NULL, NULL, confAddr, NULL);
			BC_ASSERT_PTR_NOT_NULL(berthe_conference);
			if (berthe_conference) {
				LinphoneChatRoom *berthe_chat_room = linphone_conference_get_chat_room(berthe_conference);
				BC_ASSERT_PTR_NOT_NULL(berthe_chat_room);
				if (berthe_chat_room) {
					std::string msg_text2("Hi Marie <3");
					LinphoneChatMessage *msg = ClientConference::sendTextMsg(berthe_chat_room, msg_text2);
					...
					BC_ASSERT_TRUE(wait_for_list(coresList, &berthe.getStats().number_of_LinphoneMessageSent, 1,
					                             liblinphone_tester_sip_timeout));
		...
		if (focus_conference) {
			// Check that anonymous usernames are not repeated
			int anonymous0_cnt = 0;
			int anonymous1_cnt = 0;
			int anonymous2_cnt = 0;
			bctbx_list_t *conferenceParticipants = linphone_conference_get_participant_list(focus_conference);
			BC_ASSERT_EQUAL(focus_calls_nb, linphone_conference_get_participant_count(focus_conference), int, "%d");
			for (bctbx_list_t *itp = conferenceParticipants; itp; itp = bctbx_list_next(itp)) {
				LinphoneParticipant *p = (LinphoneParticipant *)bctbx_list_get_data(itp);
				const LinphoneAddress *address = linphone_participant_get_address(p);
				const char *username = linphone_address_get_username(address);
				if (strcmp(username, "anonymous0") == 0) {
					anonymous0_cnt++;
				} else if (strcmp(username, "anonymous1") == 0) {
					anonymous1_cnt++;
				} else if (strcmp(username, "anonymous2") == 0) {
					anonymous2_cnt++;
				}
			}
			bctbx_list_free_with_data(conferenceParticipants, (void (*)(void *))linphone_participant_unref);

			BC_ASSERT_EQUAL(anonymous0_cnt, 1, int, "%d");
			BC_ASSERT_EQUAL(anonymous1_cnt, 1, int, "%d");
			BC_ASSERT_EQUAL(anonymous2_cnt, 0, int, "%d");

			auto itConferenceMgrs = std::find(conferenceMgrs.begin(), conferenceMgrs.end(), berthe.getCMgr());
			if (itConferenceMgrs != conferenceMgrs.end()) {
				conferenceMgrs.erase(itConferenceMgrs);
			}

			auto itParticipants = std::find(participants.begin(), participants.end(), berthe.getCMgr());
			if (itParticipants != participants.end()) {
				participants.erase(itParticipants);
			}

			auto itMembers = std::find(members.begin(), members.end(), berthe.getCMgr());
			if (itMembers != members.end()) {
				members.erase(itMembers);
			}

			const bctbx_list_t *participant_calls = linphone_core_get_calls(berthe.getLc());
			int participant_calls_nb = static_cast<int>(bctbx_list_size(participant_calls));
			BC_ASSERT_EQUAL(participant_calls_nb, participant_call_ended, int, "%d");

			// Berthe exits the conference
			stats focus_stat2 = focus.getStats();
			...
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallEnd,
			                             focus_stat2.number_of_LinphoneCallEnd + participant_call_ended,
			                             liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallReleased,
			                             focus_stat2.number_of_LinphoneCallReleased + participant_call_ended,
			                             liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &berthe.getStats().number_of_LinphoneCallEnd,
			                             berthe_stat2.number_of_LinphoneCallEnd + participant_call_ended,
			                             liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &berthe.getStats().number_of_LinphoneCallReleased,
			                             berthe_stat2.number_of_LinphoneCallReleased + participant_call_ended,
			                             liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &berthe.getStats().number_of_LinphoneSubscriptionTerminated,
			                             berthe_stat2.number_of_LinphoneSubscriptionTerminated + participant_call_ended,
			                             liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(
			    wait_for_list(coresList, &berthe.getStats().number_of_LinphoneConferenceStateTerminationPending,
			                  berthe_stat2.number_of_LinphoneConferenceStateTerminationPending + participant_call_ended,
			                  liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(
			    wait_for_list(coresList, &berthe.getStats().number_of_LinphoneConferenceStateTerminated,
			                  berthe_stat2.number_of_LinphoneConferenceStateTerminated + participant_call_ended,
			                  liblinphone_tester_sip_timeout));
			if (!enable_chat) {
				BC_ASSERT_TRUE(
				    wait_for_list(coresList, &berthe.getStats().number_of_LinphoneConferenceStateDeleted,
				                  berthe_stat2.number_of_LinphoneConferenceStateDeleted + participant_call_ended,
				                  liblinphone_tester_sip_timeout));
			}

			LinphoneConference *pconference =
			    linphone_core_search_conference(berthe.getLc(), NULL, berthe.getCMgr()->identity, confAddr, NULL);
			if (!!enable_chat) {
				BC_ASSERT_PTR_NOT_NULL(pconference);
			} else {
				BC_ASSERT_PTR_NULL(pconference);
			...
			LinphoneAddress *lise_identity = linphone_address_new(linphone_core_get_identity(lise.getLc()));
			participantList.insert(
			    std::make_pair(lise.getCMgr(), add_participant_info_to_list(&participants_info, lise_identity,
			                                                                LinphoneParticipantRoleSpeaker, -1)));
			...
			for (const auto &mgr : {lise.getCMgr(), berthe.getCMgr()}) {
				...
				check_call_establishment({focus, marie, laure, pauline, michelle, berthe, lise}, members,
				                         ...
				                         marie.getCMgr(), confAddr, security_level, encryption, enable_chat, FALSE,
				                         FALSE);
				...
				anonymous1_cnt = 0;
				anonymous2_cnt = 0;
				conferenceParticipants = linphone_conference_get_participant_list(focus_conference);
				BC_ASSERT_EQUAL(focus_calls_nb + idx, linphone_conference_get_participant_count(focus_conference), int,
				                "%d");
				for (bctbx_list_t *itp = conferenceParticipants; itp; itp = bctbx_list_next(itp)) {
					LinphoneParticipant *p = (LinphoneParticipant *)bctbx_list_get_data(itp);
					const LinphoneAddress *address = linphone_participant_get_address(p);
				...
				BC_ASSERT_EQUAL(anonymous0_cnt, 1, int, "%d");
				BC_ASSERT_EQUAL(anonymous1_cnt, 1, int, "%d");
				BC_ASSERT_EQUAL(anonymous2_cnt, idx, int, "%d");
				idx++;
			}
		}
		...
		focus_calls = linphone_core_get_calls(focus.getLc());
		focus_calls_nb = static_cast<int>(bctbx_list_size(focus_calls));
		BC_ASSERT_EQUAL(focus_calls_nb, 6, int, "%d");
		...
		for (auto mgr : members) {
			...
			if (mgr == berthe.getCMgr()) {
				participant_call_ended = 2;
			} else {
				participant_call_ended = 1;
			}
		...
		LinphoneConferenceInfo *focus_info =
		    linphone_core_find_conference_information_from_uri(focus.getLc(), confAddr);
		for (auto mgr : members) {
			unsigned int expected_call_logs = (mgr == berthe.getCMgr()) ? 2 : 1;
			const bctbx_list_t *call_logs = linphone_core_get_call_logs(mgr->lc);
			BC_ASSERT_EQUAL(bctbx_list_size(call_logs), expected_call_logs, size_t, "%zu");

			LinphoneAddress *pIdentity = linphone_address_new(linphone_core_get_identity(mgr->lc));
			bctbx_list_t *mgr_focus_call_log =
			    linphone_core_get_call_history_2(mgr->lc, focus.getCMgr()->identity, pIdentity);
			linphone_address_unref(pIdentity);
			BC_ASSERT_PTR_NOT_NULL(mgr_focus_call_log);
			if (mgr_focus_call_log) {
				BC_ASSERT_EQUAL(bctbx_list_size(mgr_focus_call_log), expected_call_logs, size_t, "%zu");
				for (bctbx_list_t *it = mgr_focus_call_log; it; it = bctbx_list_next(it)) {
			...
			LinphoneConferenceInfo *ref_info = linphone_conference_info_clone(focus_info);
			bctbx_list_t *participants_info2 =
			    bctbx_list_copy_with_data(linphone_conference_info_get_participant_infos(ref_info),
			                              (bctbx_list_copy_func)linphone_participant_info_clone);
			for (bctbx_list_t *it = participants_info2; it; it = bctbx_list_next(it)) {
				LinphoneParticipantInfo *participant_info = (LinphoneParticipantInfo *)bctbx_list_get_data(it);
				const LinphoneAddress *info_address = linphone_participant_info_get_address(participant_info);
				int sequence_number = 0;
				if ((mgr == laure.getCMgr()) || (mgr == berthe.getCMgr()) || (mgr == lise.getCMgr())) {
					sequence_number = -1;
				} else if (linphone_address_weak_equal(info_address, marie.getCMgr()->identity) ||
				           linphone_address_weak_equal(info_address, pauline.getCMgr()->identity) ||
				           linphone_address_weak_equal(info_address, michelle.getCMgr()->identity)) {
					sequence_number = 0;
				} else {
					sequence_number = -1;
				}
				linphone_participant_info_set_sequence_number(participant_info, sequence_number);
				if (linphone_address_weak_equal(info_address, marie.getCMgr()->identity)) {
					// In order to override all members of the organizer info. It will be added later on by
					// linphone_conference_info_set_participant_infos
					linphone_conference_info_remove_participant(ref_info, info_address);
					linphone_conference_info_set_organizer_info(ref_info, participant_info);
				}
			}

			linphone_conference_info_set_participant_infos(ref_info, participants_info2);
			bctbx_list_free_with_data(participants_info2, (bctbx_list_free_func)linphone_participant_info_unref);

			check_conference_info_against_db(mgr, confAddr, ref_info, FALSE);
			linphone_conference_info_unref(ref_info);
		}
...
static void active_speaker_conference_with_anonymous_participants(void) {
	conference_with_anonymous_participants_base(LinphoneConferenceLayoutActiveSpeaker, FALSE);
}

static void grid_conference_with_anonymous_participants(void) {
	conference_with_anonymous_participants_base(LinphoneConferenceLayoutGrid, FALSE);
}

static void conference_with_anonymous_participants_chat(void) {
#if 0
	// Disabled for the first version
	conference_with_anonymous_participants_base(LinphoneConferenceLayoutActiveSpeaker, TRUE);
#endif
}

static void client_changes_layout_in_conference_with_anonymous_participants_base(bool_t add_register_participant) {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		const LinphoneTesterLimeAlgo lime_algo = UNSET;
		...
		ClientConference marie("marie_rc", focus.getConferenceFactoryAddress(), lime_algo);
		ClientConference pauline("pauline_rc", focus.getConferenceFactoryAddress(), lime_algo);
		ClientConference laure("empty_rc", focus.getConferenceFactoryAddress(), lime_algo);
		ClientConference michelle("empty_rc", focus.getConferenceFactoryAddress(), lime_algo);
		ClientConference berthe("empty_rc", focus.getConferenceFactoryAddress(), lime_algo);
		ClientConference lise("empty_rc", focus.getConferenceFactoryAddress(), lime_algo);
		...
		// Michelle is the first element of the list as its video direction will be sendrecv. In fact as all
		// participants will turn their video capability on with a Grid layout, this will ensure that participants that
		// have a recvonly video stream, will not have any inactive stream in the SDP. In such a way, the main stream
		// will not be the first video stream
		std::list<LinphoneCoreManager *> members{michelle.getCMgr(), lise.getCMgr(), laure.getCMgr(), marie.getCMgr(),
		                                         berthe.getCMgr()};
		if (!!add_register_participant) {
			participants.push_back(pauline.getCMgr());
			members.push_back(pauline.getCMgr());
			conferenceMgrs.push_back(pauline.getCMgr());
		}
		...
		for (auto mgr : members) {
			...
			LinphoneCall *participant_call = linphone_core_get_call_by_remote_address2(mgr->lc, confAddr);
			BC_ASSERT_PTR_NOT_NULL(participant_call);
			if (participant_call && ((mgr == marie.getCMgr()) || (mgr == pauline.getCMgr()))) {
				LinphoneCallLog *call_log = linphone_call_get_call_log(participant_call);
				BC_ASSERT_TRUE(linphone_call_log_was_conference(call_log));
			}
		}
		...
		for (auto mgr : {laure.getCMgr(), berthe.getCMgr(), lise.getCMgr(), michelle.getCMgr()}) {
			LinphoneAddress *identity = linphone_address_new(linphone_core_get_identity(mgr->lc));
			participantList.insert(std::make_pair(
			    mgr, add_participant_info_to_list(&participants_info, identity, LinphoneParticipantRoleSpeaker, -1)));
			linphone_address_unref(identity);
		...
		std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
		    fill_member_list(members, participantList, marie.getCMgr(), participants_info);
		wait_for_conference_streams({focus, marie, pauline, laure, michelle, berthe, lise}, conferenceMgrs,
		                            focus.getCMgr(), memberList, confAddr, FALSE, security_level);
		...
		for (LinphoneMediaDirection default_video_direction :
		     {LinphoneMediaDirectionRecvOnly, LinphoneMediaDirectionSendRecv}) {
			for (auto participant_layout_rit = layout_sequence.rbegin();
			     participant_layout_rit != layout_sequence.rend(); ++participant_layout_rit) {
				...
				for (auto mgr_rit = members.rbegin(); mgr_rit != members.rend(); ++mgr_rit) {
					...
					linphone_call_params_set_media_encryption(new_params, encryption);
					linphone_call_params_enable_video(new_params, TRUE);
					LinphoneMediaDirection video_direction = default_video_direction;
					for (auto recvonly_mgr : recvonly_mgrs) {
						if (recvonly_mgr == mgr) {
							video_direction = LinphoneMediaDirectionRecvOnly;
						}
					...
					linphone_call_params_set_video_direction(new_params, video_direction);
					linphone_call_params_set_conference_video_layout(new_params, participant_layout);
					ms_message("%s changes its layout of conference %s to %s setting video direction to %s",
					           linphone_core_get_identity(mgr->lc), conference_address_str,
					           linphone_conference_layout_to_string(participant_layout),
					...
					BC_ASSERT_PTR_NOT_NULL(call_lparams);
					if (call_lparams) {
						BC_ASSERT_EQUAL(video_direction, linphone_call_params_get_video_direction(call_lparams), int,
						                "%i");
					}
					const LinphoneCallParams *call_cparams = linphone_call_get_current_params(participant_call);
					BC_ASSERT_PTR_NOT_NULL(call_cparams);
...
static void client_changes_layout_in_conference_with_anonymous_participants_only() {
	client_changes_layout_in_conference_with_anonymous_participants_base(FALSE);
}

static void client_changes_layout_in_conference_with_anonymous_participants() {
	client_changes_layout_in_conference_with_anonymous_participants_base(TRUE);
}
...
static test_t local_conference_scheduled_conference_with_chat_anonymous_participants_tests[] = {
    TEST_NO_TAG("Conference with anonymous participants and chat",
                LinphoneTest::conference_with_anonymous_participants_chat),
    TEST_NO_TAG("Client changes layout in conference with anonymous participants",
                LinphoneTest::client_changes_layout_in_conference_with_anonymous_participants),
    TEST_NO_TAG("Client changes layout in conference with anonymous participants only",
                LinphoneTest::client_changes_layout_in_conference_with_anonymous_participants_only)};
...
test_suite_t local_conference_test_suite_scheduled_conference_with_chat_anonymous_participants = {
    "Local conference tester (Scheduled Conference with chat and anonymous participants)",
    NULL,
    NULL,
    liblinphone_tester_before_each,
    liblinphone_tester_after_each,
    sizeof(local_conference_scheduled_conference_with_chat_anonymous_participants_tests) /
