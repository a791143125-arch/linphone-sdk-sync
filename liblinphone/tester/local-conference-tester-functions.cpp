...
static bool are_participants_camera_streams_requested(LinphoneCoreManager *mgr, LinphoneConference *conference) {
	...
	for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
		...
		if (!is_me) {
			LinphoneMediaDirection thumbnail_dir = linphone_participant_device_get_thumbnail_stream_capability(device);
...
static void check_is_me(LinphoneConference *conference,
	...
	if (conference) {
		...
		LinphoneAddress *identity = linphone_address_new(linphone_core_get_identity(core));
		...
		for (bctbx_list_t *itDevice = devices; itDevice; itDevice = bctbx_list_next(itDevice)) {
			...
			if (is_anonymous) {
				auto callId = linphone_core_find_call_log_from_call_id(
				    core, ParticipantDevice::toCpp(conf_device)->getCallId().c_str());
		...
		for (const auto &mgr : conferenceMgrs) {
			...
			if (is_focus && (focus_cleanup_period > 0)) {
				...
				BC_ASSERT_TRUE(CoreManagerAssert(coreMgrs).waitUntil(
				    chrono::seconds(focus_cleanup_period), [&mgr, &confAddr, &time_left, &focus_cleanup_period] {
					    LinphoneConferenceInfo *info =
					        linphone_core_find_conference_information_from_uri(mgr->lc, confAddr);
					    bool is_conference_info_null = (info == NULL);
					    if (info) {
						    linphone_conference_info_unref(info);
					    }
					    return (time_left > focus_cleanup_period) ? !is_conference_info_null : is_conference_info_null;
				    }));
			} else {
...
create_conference_on_server(Focus &focus,
	...
	if (is_dialout) {
		...
		// Set account after setting the capabilities so that the right factory is automatically chosen. The audio video
		// conference factory for conferences with audio and video capabilities, otherwise chat conference factory.
	...
	if (BC_ASSERT_PTR_NOT_NULL(info)) {
		...
		if (will_send_ics) {
			...
			for (auto &participant : participants) {
				linphone_conference_info_check_participant(info, participant->identity, 0);
			}
		} else {
		...
		if (!is_dialout || !previous_calls[mgr]) {
			...
			if (will_send_ics || is_dialout) {
				...
				if (itConferenceMgrs != actual_participants.end()) {
					...
					if (will_send_ics) {
						for (auto &participant : participants) {
							linphone_conference_info_check_participant(conf_info_in_db, participant->identity, 0);
						}
...
void check_call_establishment(std::list<std::reference_wrapper<CoreManager>> coreMgrs,
                              ...
                              bool_t is_dialout,
                              bool_t ics_sent) {
	int nb_participant_devices_added = static_cast<int>(members.size());
	...
	auto [focus_mgr, focus_stat] = focus;
	bctbx_list_t *coresList = NULL;
	int nb_ics_sent = 0;
	int nb_participants_added = 0;
	for (const auto &[mgr, initial_stat] : members) {
		bool account_registered = false;
		const LinphoneAccount *default_account = linphone_core_get_default_account(mgr->lc);
		if (default_account) {
			account_registered =
			    !!linphone_account_params_get_register_enabled(linphone_account_get_params(default_account));
		}
		if (account_registered && (mgr != organizer)) {
			nb_ics_sent++;
		}
	}
	for (CoreManager &core : coreMgrs) {
		coresList = bctbx_list_append(coresList, core.getCore().getCCore());
	}
	for (auto [mgr, initial_stat] : members) {
		bool is_organizer = (mgr == organizer);
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneConferenceStateCreated,
		                             initial_stat.number_of_LinphoneConferenceStateCreated + 1,
		                             liblinphone_tester_sip_timeout));
		...
		if (ics_sent && !is_dialout) {
			if (is_organizer) {
				...
				bool account_registered = false;
				const LinphoneAccount *account = linphone_conference_get_account(conference);
				if (account) {
					account_registered =
					    !!linphone_account_params_get_register_enabled(linphone_account_get_params(account));
				}
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneChatRoomStateCreated, nb_chatrooms,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_GREATER(mgr->stat.number_of_LinphoneChatRoomStateCreated, nb_chatrooms, int, "%d");
		BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneSubscriptionOutgoingProgress,
		                             initial_stat.number_of_LinphoneSubscriptionOutgoingProgress + nb_client_subscribes,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneSubscriptionActive,
		                             initial_stat.number_of_LinphoneSubscriptionActive + nb_client_subscribes,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_NotifyFullStateReceived,
		                             initial_stat.number_of_NotifyFullStateReceived + 1,
		                             liblinphone_tester_sip_timeout));
		if (security_level == LinphoneConferenceSecurityLevelEndToEnd) {
			BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_NotifyEktReceived,
			                             initial_stat.number_of_NotifyEktReceived + 1, liblinphone_tester_sip_timeout));
		}
		...
		if (participant_call) {
			BC_ASSERT_TRUE(CoreManagerAssert(coreMgrs).waitUntil(chrono::seconds(50), [&participant_call, &encryption] {
				...
				return (participant_call_enc == encryption);
			}));
		}
		...
		if ((encryption == LinphoneMediaEncryptionDTLS) || (encryption == LinphoneMediaEncryptionZRTP)) {
			...
			BC_ASSERT_EQUAL(mgr->stat.number_of_LinphoneCallEncryptedOn, initial_stat.number_of_LinphoneCallEncryptedOn,
			                int, "%d");
		}

		bctbx_list_t *focus_mgr_logs =
		    linphone_core_get_call_history_2(focus_mgr->lc, mgr->identity, focus_mgr->identity);
		if (focus_mgr_logs) {
			if (bctbx_list_size(focus_mgr_logs) < 2) {
				nb_participants_added++;
			}
			bctbx_list_free_with_data(focus_mgr_logs, (bctbx_list_free_func)linphone_call_log_unref);
		}
	}

	auto server_incoming_received = 0;
	if (is_dialout) {
		bool organizer_added = (std::find_if(members.cbegin(), members.cend(), [&organizer](auto member) {
			                        return member.first == organizer;
		                        }) != members.cend());
		if (organizer_added) {
			server_incoming_received = 1;
		}
		int server_outgoing_progress = nb_participant_devices_added - server_incoming_received;
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus_mgr->stat.number_of_LinphoneCallOutgoingProgress,
		                             focus_stat.number_of_LinphoneCallOutgoingProgress + server_outgoing_progress,
		                             liblinphone_tester_sip_timeout));
	} else {
		server_incoming_received = nb_participant_devices_added;
	}
	BC_ASSERT_TRUE(wait_for_list(coresList, &focus_mgr->stat.number_of_LinphoneCallIncomingReceived,
	                             focus_stat.number_of_LinphoneCallIncomingReceived + server_incoming_received,
	                             liblinphone_tester_sip_timeout));
	int focus_no_streams_running = 2 * nb_participant_devices_added;
	BC_ASSERT_TRUE(wait_for_list(coresList, &focus_mgr->stat.number_of_LinphoneCallUpdatedByRemote,
	                             focus_stat.number_of_LinphoneCallUpdatedByRemote +
	                                 (focus_no_streams_running - nb_participant_devices_added),
	                             liblinphone_tester_sip_timeout));
	BC_ASSERT_TRUE(wait_for_list(coresList, &focus_mgr->stat.number_of_LinphoneCallStreamsRunning,
	                             focus_stat.number_of_LinphoneCallStreamsRunning + focus_no_streams_running,
	                             liblinphone_tester_sip_timeout));

	BC_ASSERT_TRUE(CoreManagerAssert(coreMgrs).waitUntil(chrono::seconds(50), [srv_mgr = focus.first, &confAddr] {
		const LinphoneConference *focus_conference = linphone_core_search_conference_2(srv_mgr->lc, confAddr);
		if (!focus_conference) {
			return false;
		}
		return (linphone_conference_get_state(focus_conference) == LinphoneConferenceStateCreated);
	}));
	BC_ASSERT_TRUE(wait_for_list(coresList, &focus_mgr->stat.number_of_LinphoneSubscriptionIncomingReceived,
	                             focus_stat.number_of_LinphoneSubscriptionIncomingReceived +
	                                 (nb_participant_devices_added * nb_client_subscribes),
	                             liblinphone_tester_sip_timeout));
	BC_ASSERT_TRUE(wait_for_list(coresList, &focus_mgr->stat.number_of_LinphoneSubscriptionActive,
	                             focus_stat.number_of_LinphoneSubscriptionActive +
	                                 (nb_participant_devices_added * nb_client_subscribes),
	                             liblinphone_tester_sip_timeout));

	BC_ASSERT_TRUE(wait_for_list(coresList, &focus_mgr->stat.number_of_participants_added,
	                             focus_stat.number_of_participants_added + nb_participants_added,
	                             liblinphone_tester_sip_timeout));
	BC_ASSERT_TRUE(wait_for_list(coresList, &focus_mgr->stat.number_of_participant_devices_added,
	                             focus_stat.number_of_participant_devices_added + nb_participant_devices_added,
	                             liblinphone_tester_sip_timeout));
	BC_ASSERT_TRUE(
	    wait_for_list(coresList, &focus_mgr->stat.number_of_conference_participant_devices_present,
	                  focus_stat.number_of_conference_participant_devices_present + nb_participant_devices_added,
	                  liblinphone_tester_sip_timeout));
	BC_ASSERT_TRUE(wait_for_list(coresList, &focus_mgr->stat.number_of_participant_devices_present,
	                             focus_stat.number_of_participant_devices_present + nb_participant_devices_added,
	                             liblinphone_tester_sip_timeout));

	if ((encryption == LinphoneMediaEncryptionDTLS) || (encryption == LinphoneMediaEncryptionZRTP)) {
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus_mgr->stat.number_of_LinphoneCallEncryptedOn,
		                             focus_stat.number_of_LinphoneCallEncryptedOn + nb_participant_devices_added,
		                             liblinphone_tester_sip_timeout));
	}
	...
	for (auto [mgr, initial_stat] : members) {
		...
		BC_ASSERT_PTR_NOT_NULL(participant_call);
		if (participant_call) {
			LinphoneCall *focus_call = get_peer_call(focus_mgr, participant_call);
			BC_ASSERT_PTR_NOT_NULL(focus_call);
			if (focus_call) {
				const LinphoneCallParams *call_cparams = linphone_call_get_current_params(focus_call);
				const LinphoneMediaEncryption focus_call_enc = linphone_call_params_get_media_encryption(call_cparams);
				BC_ASSERT_EQUAL(focus_call_enc, encryption, int, "%d");
			}
		}
	}

	for (auto mgr : membersMgr) {
		LinphoneAddress *deviceAddr = linphone_account_get_contact_address(linphone_core_get_default_account(mgr->lc));
		LinphoneChatRoom *core_chat_room = linphone_core_search_chat_room(mgr->lc, NULL, deviceAddr, confAddr, NULL);
		if (!!enable_chat) {
		...
		LinphoneConference *pconference = linphone_core_search_conference(mgr->lc, NULL, mgr->identity, confAddr, NULL);
	...
	// Ensure that for end-to-end encrypted conferences, the server receives at least one PUBLISH. In fact, depending on
	// network conditions and how much participants are joining apart, only the first one might be sending the CSPI,
	// SSPI and EKT keys. Anyway, function does_all_participants_have_matching_ekt will make sure that all conferences
	// are sync'ed up
	if (security_level == LinphoneConferenceSecurityLevelEndToEnd) {
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus_mgr->stat.number_of_LinphonePublishIncomingReceived, 1,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(
		    wait_for_list(coresList, &focus_mgr->stat.number_of_LinphonePublishOk, 1, liblinphone_tester_sip_timeout));
	}

	BC_ASSERT_TRUE(
	    CoreManagerAssert(coreMgrs).waitUntil(chrono::seconds(50), [&membersMgr, &confAddr, &security_level] {
		    return does_all_participants_have_matching_ekt(membersMgr, confAddr, security_level);
	    }));
...
bool does_all_participants_have_matching_ekt(std::list<LinphoneCoreManager *> members,
                                             ...
                                             LinphoneConferenceSecurityLevel security_level) {
	if (security_level != LinphoneConferenceSecurityLevelEndToEnd) {
		bool hasClientEktManager = true;
		for (auto member : members) {
			auto conf = linphone_core_search_conference_2(member->lc, confAddr);
			if (!conf) {
				return false;
			}
			...
			// No need to check SSPI, CSPI and EKT if the conference is not encrypted
			hasClientEktManager = (rcConf->getClientEktManager() != nullptr);
		}
		// The client ekt manager should not be created if the conference is not end-to-end encrypted
		return !hasClientEktManager;
	}

	auto firstClientCConf = linphone_core_search_conference_2(members.front()->lc, confAddr);
	if (!firstClientCConf) {
		return false;
	}
	...
	for (auto member : members) {
		...
		sspiOk &= (firstClientEktCtx->getSSpi() == rcEktCtx->getSSpi());
		cspiOk &= (firstClientEktCtx->getCSpi() == rcEktCtx->getCSpi());
		ektOk &= (firstClientEktCtx->getEkt() == rcEktCtx->getEkt());
	}
	return sspiOk && cspiOk && ektOk;
}
#endif // HAVE_ADVANCED_IM
...
void check_muted(std::initializer_list<std::reference_wrapper<CoreManager>> coreMgrs,
	...
	BC_ASSERT_TRUE(CoreManagerAssert(coreMgrs).waitUntil(chrono::seconds(10), [&device, &expect_mute] {
		return ((!!linphone_participant_device_get_is_muted(device)) == expect_mute);
	}));
}
		...
		CoreManagerAssert(coreMgrs).waitUntil(chrono::seconds(50), [mgr, &focus, &members, confAddr, enable_video,
			...
			if (mgr == focus) {
				for (const auto &[memberMgr, info] : members) {
					LinphoneCall *participant_call = linphone_core_get_call_by_remote_address2(memberMgr->lc, confAddr);
					call_check &= (participant_call != nullptr) &&
					              (linphone_call_get_state(participant_call) == LinphoneCallStateStreamsRunning);
					LinphoneCall *focus_call = get_peer_call(focus, participant_call);
					call_check &= (focus_call != nullptr) &&
					              (linphone_call_get_state(focus_call) == LinphoneCallStateStreamsRunning);
					if (focus_call) {
						calls.push_back(focus_call);
					}
				}
			} else {
				LinphoneCall *participant_call = linphone_core_get_call_by_remote_address2(mgr->lc, confAddr);
				call_check &= (participant_call != nullptr) &&
				              (linphone_call_get_state(participant_call) == LinphoneCallStateStreamsRunning);
				if (participant_call) {
					calls.push_back(participant_call);
					const LinphoneCall *focus_call = get_peer_call(focus, participant_call);
					call_check &= (focus_call != nullptr) &&
					              (linphone_call_get_state(focus_call) == LinphoneCallStateStreamsRunning);
				}
			}
			call_check &= (!calls.empty());
			...
			if (conference) {
				...
				for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
					...
					LinphoneParticipant *participant = linphone_participant_device_get_participant(device);
						...
						try {
							auto participant_mgr_it =
							    std::find_if(conferenceMgrs.cbegin(), conferenceMgrs.cend(),
							                 [focus, device_address, confAddr](const LinphoneCoreManager *mgr) {
								                 LinphoneCall *participant_call =
								                     linphone_core_get_call_by_remote_address2(mgr->lc, confAddr);
								                 if (!participant_call) return false;
								                 LinphoneCall *focus_call = get_peer_call(focus, participant_call);
								                 if (!focus_call) return false;
								                 const LinphoneAddress *contact_address =
								                     linphone_call_get_remote_contact_address(focus_call);
								                 return !!linphone_address_weak_equal(contact_address, device_address);
							                 });
					...
					if (found) {
						...
						device_check &= (linphone_participant_device_is_in_conference(device) ? true : false);
					} else {
						expected_state = LinphoneParticipantDeviceStateLeft;
						device_check &= (linphone_participant_device_is_in_conference(device) ? false : true);
					}
					...
					if (call) {
						...
						bool_t thumbnail_available =
						    linphone_participant_device_get_thumbnail_stream_availability(device);
						const LinphoneCallParams *call_params = linphone_call_get_params(call);
						bool_t call_video_enabled = linphone_call_params_video_enabled(call_params);
						bool_t video_available =
						    linphone_participant_device_get_stream_availability(device, LinphoneStreamTypeVideo);
						if (enable_video && ((mgr == focus) || call_video_enabled)) {
							...
							if (!is_thumbnail_inactive) {
								bool_t is_screen_sharing = linphone_participant_device_screen_sharing_enabled(device);
								const char *video_label =
								    linphone_participant_device_get_stream_label(device, LinphoneStreamTypeVideo);
								const char *thumbnail_label =
								    linphone_participant_device_get_thumbnail_stream_label(device);
				...
				for (const auto &[memberMgr, info] : members) {
					LinphoneConference *mgr_conference = linphone_core_search_conference_2(memberMgr->lc, confAddr);
					const LinphoneAccount *default_account = NULL;
					if (mgr_conference) {
						default_account = linphone_conference_get_account(mgr_conference);
					} else {
			...
			return audio_direction_check && video_check && device_check && call_check && participant_check &&
			       security_check && participant_is_me_check && participant_device_is_me_check;
		});
...
#ifdef HAVE_ADVANCED_IM
	std::list<LinphoneCoreManager *> membersMgr;
	for (const auto &[mgr, info] : members) {
		membersMgr.push_back(mgr);
	}
	BC_ASSERT_TRUE(
	    CoreManagerAssert(coreMgrs).waitUntil(chrono::seconds(50), [&membersMgr, &confAddr, &security_level] {
		    return does_all_participants_have_matching_ekt(membersMgr, confAddr, security_level);
...
bool_t check_thumbnail_availability(const LinphoneCoreManager *mgr, const LinphoneAddress *confAddr) {
	LinphoneConference *conference = linphone_core_search_conference_2(mgr->lc, confAddr);
	bctbx_list_t *devices = linphone_conference_get_participant_device_list(conference);
	bool_t thumbnail_availability_ok = TRUE;
	for (const bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
		auto device = static_cast<LinphoneParticipantDevice *>(bctbx_list_get_data(itd));
		bool video_available = !!linphone_participant_device_get_stream_availability(device, LinphoneStreamTypeVideo);
		const LinphoneParticipant *participant = linphone_participant_device_get_participant(device);
		const LinphoneParticipantRole role = linphone_participant_get_role(participant);
		// There is no thumbnail if video is not enabled or the participant is not a speaker
		bool_t expected_thumbnail_available = (video_available && (role == LinphoneParticipantRoleSpeaker));
		bool_t thumbnail_available = linphone_participant_device_get_thumbnail_stream_availability(device);
		thumbnail_availability_ok &= (thumbnail_available == expected_thumbnail_available);
	}
...
void toggle_screen_sharing(std::list<std::reference_wrapper<CoreManager>> coreMgrs,
	...
	if (!screen_sharing_mgr_new_call_params) {
		return;
	}
	stats focus_stat = focus->stat;
	stats screen_sharing_mgr_stat = screen_sharing_mgr->stat;
	std::list<std::pair<LinphoneCoreManager *, stats>> members_stat_pair;
	bctbx_list_t *coresList = NULL;
	for (CoreManager &core : coreMgrs) {
	...
	for (const auto &mgr : members) {
		members_stat_pair.push_back(std::make_pair(mgr, mgr->stat));
	}

	bool_t video_enabled = linphone_call_params_video_enabled(screen_sharing_mgr_new_call_params);
	bool_t screen_sharing_enabled = linphone_call_params_screen_sharing_enabled(screen_sharing_mgr_new_call_params);
	LinphoneMediaDirection video_direction =
	    linphone_call_params_get_video_direction(screen_sharing_mgr_new_call_params);
	...
	if (screen_sharing_mgr_call) {
		...
		linphone_call_set_video_source(screen_sharing_mgr_call, descriptor);
		linphone_video_source_descriptor_unref(descriptor);

		BC_ASSERT_TRUE(wait_for_list(coresList, &screen_sharing_mgr->stat.number_of_LinphoneCallUpdating,
		                             screen_sharing_mgr_stat.number_of_LinphoneCallUpdating + 1,
		                             liblinphone_tester_sip_timeout));

		BC_ASSERT_TRUE(wait_for_list(coresList, &screen_sharing_mgr->stat.number_of_LinphoneCallStreamsRunning,
		                             screen_sharing_mgr_stat.number_of_LinphoneCallStreamsRunning + 1,
		                             liblinphone_tester_sip_timeout));
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus->stat.number_of_LinphoneCallStreamsRunning,
		                             ...
		                             liblinphone_tester_sip_timeout));
		if (!!screen_sharing_allowed) {
			BC_ASSERT_TRUE(check_screen_sharing_call(focus, screen_sharing_mgr, offer_screen_sharing));
		}
	}

	if (screen_sharing_allowed) {
		LinphoneCoreManager *new_screen_sharing_mgr = NULL;
		if (!!video_enabled && !!screen_sharing_enabled) {
			new_screen_sharing_mgr = screen_sharing_mgr;
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus->stat.number_of_participant_devices_screen_sharing_enabled,
			                             focus_stat.number_of_participant_devices_screen_sharing_enabled + 1,
			                             liblinphone_tester_sip_timeout));
			for (const auto &[mgr, initial_stat] : members_stat_pair) {
				BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_participant_devices_screen_sharing_enabled,
				                             initial_stat.number_of_participant_devices_screen_sharing_enabled + 1,
				                             liblinphone_tester_sip_timeout));
			}
		} else {
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus->stat.number_of_participant_devices_screen_sharing_disabled,
			                             focus_stat.number_of_participant_devices_screen_sharing_disabled + 1,
			                             liblinphone_tester_sip_timeout));
			for (const auto &[mgr, initial_stat] : members_stat_pair) {
				BC_ASSERT_TRUE(wait_for_list(coresList,
				                             &mgr->stat.number_of_participant_devices_screen_sharing_disabled,
				                             initial_stat.number_of_participant_devices_screen_sharing_disabled + 1,
				                             liblinphone_tester_sip_timeout));
			}
		}

		auto conferenceMgrs = members;
		conferenceMgrs.push_back(focus);
		for (const auto &mgr : conferenceMgrs) {
			BC_ASSERT_TRUE(
			    CoreManagerAssert(coreMgrs).waitUntil(chrono::seconds(50), [mgr, &new_screen_sharing_mgr, confAddr] {
				    return check_screen_sharing(mgr, confAddr, new_screen_sharing_mgr);
			    }));
		}
	} else {
		BC_ASSERT_FALSE(wait_for_list(coresList, &focus->stat.number_of_participant_devices_screen_sharing_enabled,
		                              focus_stat.number_of_participant_devices_screen_sharing_enabled + 1, 3000));
		for (const auto &[mgr, initial_stat] : members_stat_pair) {
			BC_ASSERT_FALSE(wait_for_list(coresList, &mgr->stat.number_of_participant_devices_screen_sharing_enabled,
			                              initial_stat.number_of_participant_devices_screen_sharing_enabled + 1, 100));
		}
	}

	bool_t screen_sharing_core_video_enabled = !!linphone_core_video_display_enabled(screen_sharing_mgr->lc) ||
	                                           !!linphone_core_video_capture_enabled(screen_sharing_mgr->lc);
	bool_t expect_screen_sharing = screen_sharing_allowed && offer_screen_sharing &&
	                               screen_sharing_core_video_enabled && direction_ok_for_screen_sharing;
	bool_t expect_video_enabled = screen_sharing_core_video_enabled && video_enabled && direction_ok_for_screen_sharing;
	if (screen_sharing_mgr_call) {
		const LinphoneCallParams *call_cparams = linphone_call_get_current_params(screen_sharing_mgr_call);
		BC_ASSERT_EQUAL(linphone_call_params_video_enabled(call_cparams), expect_video_enabled, int, "%0d");
		BC_ASSERT_EQUAL(linphone_call_params_screen_sharing_enabled(call_cparams), expect_screen_sharing, int, "%0d");
	}

	const LinphoneCall *focus_call = get_peer_call(focus, screen_sharing_mgr_call);
	BC_ASSERT_PTR_NOT_NULL(focus_call);
	if (focus_call) {
		const LinphoneCallParams *call_cparams = linphone_call_get_current_params(focus_call);
		BC_ASSERT_EQUAL(linphone_call_params_video_enabled(call_cparams), expect_video_enabled, int, "%0d");
		...
		BC_ASSERT_EQUAL(linphone_call_params_video_enabled(call_rparams),
		                video_enabled && direction_ok_for_screen_sharing, int, "%0d");
		BC_ASSERT_EQUAL(linphone_call_params_screen_sharing_enabled(call_rparams),
		                screen_sharing_core_video_enabled && offer_screen_sharing, int, "%0d");
	}
	bctbx_list_free(coresList);
}
...
bool_t check_screen_sharing(const LinphoneCoreManager *mgr,
	...
	const LinphoneAddress *screen_sharing_device_address = NULL;
	bool_t screen_sharing_ok = TRUE;
	// Get the screen sharing device address.
	// Try first to get the contact address of the account associated with the conference. However, cores without a
	// registered account can join a conference, hence secondly use the "me" device address
	if (screen_sharing_mgr) {
		LinphoneConference *screen_sharing_conference =
		    linphone_core_search_conference_2(screen_sharing_mgr->lc, confAddr);
		if (screen_sharing_conference) {
			LinphoneAccount *screen_sharing_conference_account =
			    linphone_conference_get_account(screen_sharing_conference);
			if (screen_sharing_conference_account) {
				screen_sharing_device_address = linphone_account_get_contact_address(screen_sharing_conference_account);
			}
			if (!screen_sharing_device_address) {
				LinphoneParticipant *screen_sharing_me_participant =
				    linphone_conference_get_me(screen_sharing_conference);
				if (screen_sharing_me_participant) {
					bctbx_list_t *screen_sharing_me_devices =
					    linphone_participant_get_devices(screen_sharing_me_participant);
					screen_sharing_ok &= (bctbx_list_size(screen_sharing_me_devices) == 1);
					LinphoneParticipantDevice *screen_sharing_me_device =
					    static_cast<LinphoneParticipantDevice *>(bctbx_list_get_data(screen_sharing_me_devices));
					screen_sharing_device_address = linphone_participant_device_get_address(screen_sharing_me_device);
					bctbx_list_free_with_data(screen_sharing_me_devices,
					                          (bctbx_list_free_func)linphone_participant_device_unref);
				} else {
					return FALSE;
				}
			}
	...
	const LinphoneConference *conference = linphone_core_search_conference_2(mgr->lc, confAddr);
	const LinphoneParticipantDevice *screen_sharing_device =
	    linphone_conference_get_screen_sharing_participant_device(conference);
	const LinphoneParticipant *screen_sharing_participant =
	    linphone_conference_get_screen_sharing_participant(conference);
	bctbx_list_t *devices = linphone_conference_get_participant_device_list(conference);
	for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
		LinphoneParticipantDevice *device = static_cast<LinphoneParticipantDevice *>(bctbx_list_get_data(itd));
		const LinphoneAddress *device_address = linphone_participant_device_get_address(device);
		bool_t is_screen_sharing_mgr =
		    screen_sharing_mgr ? linphone_address_weak_equal(device_address, screen_sharing_device_address) : FALSE;
		bool_t is_screen_sharing = linphone_participant_device_screen_sharing_enabled(device);
		screen_sharing_ok &= (is_screen_sharing == is_screen_sharing_mgr);
		LinphoneParticipant *participant = linphone_participant_device_get_participant(device);
		if (is_screen_sharing_mgr) {
			screen_sharing_ok &= (device == screen_sharing_device);
		...
		bool_t thumbnail_available = linphone_participant_device_get_thumbnail_stream_availability(device);
		if (thumbnail_available) {
			const char *video_label = linphone_participant_device_get_stream_label(device, LinphoneStreamTypeVideo);
			const char *thumbnail_label = linphone_participant_device_get_thumbnail_stream_label(device);
			// If screensharing, the main video and thumbnail labels are different. The are the same otherwise
			if (is_screen_sharing) {
...
void set_video_settings_in_conference(LinphoneCoreManager *focus,
	...
	LinphoneCall *participant_call = linphone_core_get_call_by_remote_address2(participantMgr->lc, confAddr);
	BC_ASSERT_PTR_NOT_NULL(participant_call);
	if (participant_call) {
		...
		ms_message("%s %s video with direction %s", linphone_core_get_identity(participantMgr->lc),
		           ((enable_video) ? "enables" : "disables"), linphone_media_direction_to_string(video_direction));

		LinphoneCallParams *new_params = linphone_core_create_call_params(participantMgr->lc, participant_call);
	...
	if (focus_defer_update == TRUE) {
		LinphoneAddress *uri = participantMgr->identity;
	...
	for (auto mgr : active_cores) {
		...
		if (pconference) {
			LinphoneParticipant *participant =
			    (mgr == participantMgr) ? linphone_conference_get_me(pconference)
			                            : linphone_conference_find_participant(pconference, participantMgr->identity);
			...
			if (participant) {
				...
				for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
					...
					BC_ASSERT_EQUAL(linphone_participant_device_get_stream_capability(device, LinphoneStreamTypeVideo),
	...
	participant_call = linphone_core_get_call_by_remote_address2(participantMgr->lc, confAddr);
	BC_ASSERT_PTR_NOT_NULL(participant_call);
	if (participant_call) {
...
void create_conference_base(time_t start_time,
	...
	{ // to make sure focus is destroyed after clients.
		...
		const LinphoneTesterLimeAlgo lime_algo_to_use = encrypted_conference ? lime_algo : UNSET;
		if (encrypted_conference && lime_algo_to_use == UNSET) {
		...
		for (auto mgr : members) {
			if (!version_mismatch || (mgr != laure.getCMgr())) {
			...
			linphone_call_params_unref(new_params);
			LinphoneCall *participant_call = linphone_core_get_call_by_remote_address2(mgr->lc, confAddr2);
			BC_ASSERT_PTR_NOT_NULL(participant_call);
			if (participant_call) {
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, encryption, enable_chat, (start_time < 0), TRUE);
		std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
		...
		for (auto mgr : conferenceMgrs) {
			...
			BC_ASSERT_PTR_NOT_NULL(pconference);
			if (pconference) {
				if (enable_video && !((layout != LinphoneConferenceLayoutGrid) &&
				                      (video_direction == LinphoneMediaDirectionRecvOnly))) {
					...
					if (!(mgr == focus.getCMgr() && security_level == LinphoneConferenceSecurityLevelEndToEnd)) {
						for (bctbx_list_t *d_it = participant_device_list; d_it; d_it = bctbx_list_next(d_it)) {
							...
							if (device) {
								...
								linphone_participant_device_add_callbacks(device, cbs);
				...
				if (mgr == focus.getCMgr()) {
					...
					BC_ASSERT_PTR_NOT_NULL(focus_call);
					if (focus_call) {
						_linphone_call_check_nb_active_streams(focus_call, no_streams_audio, no_active_streams_video,
						                                       no_streams_text);
						if (!enable_ice) {
							_linphone_call_check_max_nb_streams(focus_call, no_streams_audio, no_streams_video,
							                                    no_streams_text);
							const LinphoneCallParams *call_lparams = linphone_call_get_params(focus_call);
							BC_ASSERT_EQUAL(linphone_call_params_video_enabled(call_lparams),
				...
				for (bctbx_list_t *itp = participants; itp; itp = bctbx_list_next(itp)) {
					...
					BC_ASSERT_TRUE(linphone_participant_is_admin(participant) ==
					               linphone_address_weak_equal(linphone_participant_get_address(participant),
		...
		if (client_restart) {
			...
			std::list<std::pair<LinphoneCoreManager *, stats>> new_member_stats_list{
			    std::make_pair(marie.getCMgr(), marie.getStats())};
			...
			check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
			                         std::make_pair(focus.getCMgr(), focus_stat), new_member_stats_list,
			                         marie.getCMgr(), confAddr, security_level, encryption, enable_chat,
			                         (start_time < 0), TRUE);
				...
				if (pconference) {
					...
					if (mgr == focus.getCMgr()) {
						...
						if (focus_call) {
							_linphone_call_check_max_nb_streams(focus_call, no_streams_audio, no_streams_video,
							                                    no_streams_text);
							_linphone_call_check_nb_active_streams(focus_call, no_streams_audio,
							                                       no_active_streams_video, no_streams_text);
							const LinphoneCallParams *call_lparams = linphone_call_get_params(focus_call);
							BC_ASSERT_EQUAL(linphone_call_params_video_enabled(call_lparams), enabled, int, "%0d");
							const LinphoneCallParams *call_rparams = linphone_call_get_remote_params(focus_call);
				...
				try {
					...
					if (pauline_call) {
						...
						for (auto mgr : conferenceMgrs) {
							...
							BC_ASSERT_PTR_NOT_NULL(pconference);
							if (pconference) {
								LinphoneParticipant *participant = (mgr == pauline.getCMgr())
								...
								if (participant) {
									...
									for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
										...
										BC_ASSERT_EQUAL(linphone_participant_device_get_stream_capability(
										                    device, LinphoneStreamTypeVideo),
										                expected_video_direction, int, "%0d");
									}
									if (devices) {
										bctbx_list_free_with_data(
										    devices, (bctbx_list_free_func)linphone_participant_device_unref);
									}
								}
							}
						}
								...
								if (mgr == pauline.getCMgr()) {
									...
									if (participant) {
										...
										for (bctbx_list_t *it_d = devices; it_d != NULL; it_d = it_d->next) {
											LinphoneParticipantDevice *device = (LinphoneParticipantDevice *)it_d->data;
											BC_ASSERT_PTR_NOT_NULL(device);
											if (device) {
												BC_ASSERT_EQUAL(linphone_participant_device_get_state(device),
					...
					if (mgr != michelle.getCMgr()) {
						...
						if (michelle_participant) {
							...
							for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
								LinphoneParticipantDevice *device =
								    (LinphoneParticipantDevice *)bctbx_list_get_data(itd);
								BC_ASSERT_EQUAL((int)linphone_participant_device_get_state(device),
				...
				for (auto mgr : conferenceMgrs) {
					...
					if (pconference) {
						...
						if (mgr == focus.getCMgr()) {
							...
							BC_ASSERT_PTR_NOT_NULL(focus_call);
							if (focus_call) {
								_linphone_call_check_max_nb_streams(focus_call, no_max_streams_audio, no_streams_video,
								                                    no_streams_text);
								_linphone_call_check_nb_active_streams(focus_call, no_streams_audio,
								                                       no_active_streams_video, no_streams_text);
							}
...
void create_conference_with_screen_sharing_base(time_t start_time,
	...
	{ // to make sure focus is destroyed after clients.
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, encryption, FALSE, (start_time < 0), TRUE);
		std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
		    fill_member_list(members, participantList, marie.getCMgr(), participants_info);
		wait_for_conference_streams({focus, marie, pauline, laure, michelle, berthe}, conferenceMgrs, focus.getCMgr(),
		                            memberList, confAddr, enable_video, security_level);
			    ...
			    CoreManagerAssert({focus, marie, pauline, laure, michelle, berthe})
			        .waitUntil(chrono::seconds(50), [mgr, enable_video, video_direction, confAddr] {
				        ...
				        for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
					        ...
					        if (participant) {
						        bool_t video_available = linphone_participant_device_get_stream_availability(
						            device, LinphoneStreamTypeVideo);
						        if (enable_video &&
						            ((video_direction == LinphoneMediaDirectionSendOnly) ||
						             (video_direction == LinphoneMediaDirectionSendRecv)) &&
		...
		if (berthe_call) {
			...
			toggle_screen_sharing({focus, marie, pauline, laure, michelle, berthe}, focus.getCMgr(), members,
			                      berthe.getCMgr(), new_params, confAddr, berthe_can_screen_share);
			linphone_call_params_unref(new_params);
		}
		...
		if (laure_call) {
			...
			linphone_call_params_enable_screen_sharing(new_params, TRUE);
			toggle_screen_sharing({focus, marie, pauline, laure, michelle, berthe}, focus.getCMgr(), members,
			                      laure.getCMgr(), new_params, confAddr, FALSE);
			linphone_call_params_unref(new_params);
		}
		...
		LinphoneCoreManager *screen_sharing_mgr = (berthe_can_screen_share) ? berthe.getCMgr() : NULL;
		for (const auto &mgr : conferenceMgrs) {
			BC_ASSERT_TRUE(check_screen_sharing(mgr, confAddr, screen_sharing_mgr));
		}
		...
		if (enable_video && clients_have_video_send_component) {
			...
			for (const auto &mgr : conferenceMgrs) {
				BC_ASSERT_TRUE(check_screen_sharing(mgr, confAddr, berthe.getCMgr()));
			}
		}
		...
		for (const auto &mgr : conferenceMgrs) {
			...
			for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
				...
				if (is_berthe && berthe_can_screen_share) {
					BC_ASSERT_PTR_EQUAL(device, screen_sharing_device);
					BC_ASSERT_PTR_EQUAL(participant, screen_sharing_participant);
				} else {
					BC_ASSERT_PTR_NOT_EQUAL(device, screen_sharing_device);
		...
		std::list<std::pair<LinphoneCoreManager *, stats>> one_member_stats_list{
		    std::make_pair(michelle.getCMgr(), michelle.getStats())};
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), one_member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, encryption, FALSE, FALSE, TRUE);
...
void create_conference_with_screen_sharing_chat_base(time_t start_time,
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto mgr : members) {
			...
			BC_ASSERT_PTR_NOT_NULL(participant_call);
			if (participant_call) {
				linphone_call_set_video_source(participant_call, descriptor);
				LinphoneCallLog *call_log = linphone_call_get_call_log(participant_call);
				BC_ASSERT_TRUE(linphone_call_log_was_conference(call_log));
			}
			linphone_video_source_descriptor_unref(descriptor);
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, encryption, TRUE, (start_time < 0), TRUE);
		std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
		    fill_member_list(members, participantList, marie.getCMgr(), participants_info);
		wait_for_conference_streams({focus, marie, pauline, laure, michelle, berthe}, conferenceMgrs, focus.getCMgr(),
			        ...
			        .waitUntil(chrono::seconds(50), [mgr, &pauline, confAddr] {
				        ...
				        for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
					        ...
					        if (participant) {
						        bool_t video_available = linphone_participant_device_get_stream_availability(
						            device, LinphoneStreamTypeVideo);
						        if (linphone_participant_get_role(participant) == LinphoneParticipantRoleSpeaker) {
							        video_ok &= video_available;
						        } else {
							        video_ok &= !video_available;
						        }
		...
		std::list<std::pair<LinphoneCoreManager *, stats>> one_member_stats_list{
		    std::make_pair(pauline.getCMgr(), pauline.getStats())};
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), one_member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, encryption, TRUE, FALSE, TRUE);
		...
		if (!rejoin_with_screen_sharing) {
			...
			if (pauline_call) {
				...
				linphone_call_params_enable_screen_sharing(screen_sharing_new_params, TRUE);
				toggle_screen_sharing({focus, marie, pauline, laure, michelle, berthe}, focus.getCMgr(), members,
				                      pauline.getCMgr(), screen_sharing_new_params, confAddr, TRUE);
				linphone_call_params_unref(screen_sharing_new_params);
		...
		for (auto mgr : members) {
			...
			for (auto remaining_mgr : remaining_members) {
				remaining_members_stats.push_back(remaining_mgr->stat);
				LinphoneCall *participant_call = linphone_core_get_call_by_remote_address2(remaining_mgr->lc, confAddr);
				BC_ASSERT_PTR_NOT_NULL(participant_call);
				if (participant_call) {
					const LinphoneCallParams *call_cparams = linphone_call_get_current_params(participant_call);
					if (linphone_call_params_video_enabled(call_cparams)) {
...
void create_conference_with_late_participant_addition_base(time_t start_time,
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto &participant : participants) {
			participantList.insert(std::make_pair(
			    participant, add_participant_info_to_list(&participants_info, participant->identity, role, -1)));
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, LinphoneMediaEncryptionNone, FALSE, is_dialout, TRUE);
		...
		for (auto mgr : conferenceMgrs) {
			...
			if (pconference) {
				...
				if (mgr == focus.getCMgr()) {
					...
					BC_ASSERT_PTR_NOT_NULL(focus_call);
					if (focus_call) {
						_linphone_call_check_max_nb_streams(focus_call, no_streams_audio, no_max_streams_video,
						                                    no_streams_text);
						_linphone_call_check_nb_active_streams(focus_call, no_streams_audio, no_streams_video,
						                                       no_streams_text);
						const LinphoneCallParams *call_lparams = linphone_call_get_params(focus_call);
						BC_ASSERT_EQUAL(linphone_call_params_video_enabled(call_lparams), enabled, int, "%0d");
						const LinphoneCallParams *call_rparams = linphone_call_get_remote_params(focus_call);
		...
		std::list<std::pair<LinphoneCoreManager *, stats>> new_member_stats_list{
		    std::make_pair(berthe.getCMgr(), berthe.getStats())};
		if (one_addition) {
			linphone_conference_add_participant_2(oconference, berthe.getCMgr()->identity);
		} else {
			new_member_stats_list.push_back(std::make_pair(michelle.getCMgr(), michelle.getStats()));
		...
		if (accept) {
			ms_message("%s accepts the call to rejoin conference %s", linphone_core_get_identity(berthe.getLc()),
			...
			if (!one_addition) {
				ms_message("%s accepts the call to rejoin conference %s", linphone_core_get_identity(michelle.getLc()),
				           Address::toCpp(confAddr)->toString().c_str());
				if (michelle_call) {
		...
		int nb_subscriptions = (encrypted_conference) ? 2 : 1;
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), one_member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, LinphoneMediaEncryptionNone, FALSE, FALSE, !is_dialout);
...
void create_conference_with_chat_base(LinphoneConferenceSecurityLevel security_level,
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto mgr : {focus.getCMgr(), marie.getCMgr(), pauline.getCMgr(), laure.getCMgr(), michelle.getCMgr(),
			...
			if (mgr == focus.getCMgr()) {
				// ICE relay candidates will only be tested with Michelle
				linphone_core_set_user_agent(mgr->lc, "Natted Linphone", NULL);
			} else {
				linphone_core_set_default_conference_layout(mgr->lc, LinphoneConferenceLayoutActiveSpeaker);
				linphone_core_set_media_encryption(mgr->lc, encrypted_conference ? LinphoneMediaEncryptionZRTP
				                                                                 : LinphoneMediaEncryptionNone);
		...
		const LinphoneMediaEncryption encryption =
		    encrypted_conference ? LinphoneMediaEncryptionZRTP : LinphoneMediaEncryptionNone;
		std::list<std::pair<LinphoneCoreManager *, stats>> member_stats_list;
		for (auto mgr : members) {
			member_stats_list.push_back(std::make_pair(mgr, mgr->stat));
			LinphoneCallParams *new_params = linphone_core_create_call_params(mgr->lc, nullptr);
		...
		for (auto mgr : members) {
			...
			if (focus_call) {
				...
				const LinphoneMediaEncryption focus_call_enc = linphone_call_params_get_media_encryption(call_cparams);
				BC_ASSERT_EQUAL(focus_call_enc, encryption, int, "%d");
				LinphoneCallLog *call_log = linphone_call_get_call_log(focus_call);
				BC_ASSERT_PTR_NOT_NULL(call_log);
				if (call_log) {
					BC_ASSERT_TRUE(linphone_address_weak_equal(linphone_call_log_get_from_address(call_log), from));
					BC_ASSERT_TRUE(linphone_address_weak_equal(linphone_call_log_get_to_address(call_log), to));
				...
				if (chat_room) {
					...
					if (chat_params) {
						LinphoneChatRoomEncryptionBackend encryption_backend =
						    encrypted_conference ? LinphoneChatRoomEncryptionBackendLime
						...
						BC_ASSERT_EQUAL(linphone_chat_params_get_encryption_backend(chat_params), encryption_backend,
						                int, "%d");
						BC_ASSERT_EQUAL(linphone_chat_params_encryption_enabled(chat_params), !!encrypted_conference,
						                int, "%d");
					}
				}
		...
		int nb_client_subscribes = (encrypted_conference) ? 2 : 1;
		if (!!client_reenter_conference) {
			...
			const std::list<std::pair<LinphoneCoreManager *, stats>> new_member_stats_list{
			    std::make_pair(marie.getCMgr(), marie_stat)};
			LinphoneCallParams *new_params = linphone_core_create_call_params(marie.getLc(), nullptr);
			linphone_call_params_set_media_encryption(new_params, encryption);
			linphone_call_params_set_video_direction(new_params, LinphoneMediaDirectionSendRecv);
			...
			linphone_core_invite_address_with_params_2(marie.getLc(), confAddr, new_params, nullptr, nullptr);
			linphone_call_params_unref(new_params);
			LinphoneCall *participant_call = linphone_core_get_call_by_remote_address2(marie.getLc(), confAddr);
			BC_ASSERT_PTR_NOT_NULL(participant_call);
			if (participant_call) {
				LinphoneCallLog *call_log = linphone_call_get_call_log(participant_call);
				BC_ASSERT_TRUE(linphone_call_log_was_conference(call_log));
				check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
				                         std::make_pair(focus.getCMgr(), focus_stat), new_member_stats_list,
				                         marie.getCMgr(), confAddr, security_level, encryption, TRUE, (start_time < 0),
				                         TRUE);
				wait_for_conference_streams({focus, marie, pauline, laure, michelle, berthe}, conferenceMgrs,
		...
		for (auto mgr : {marie.getCMgr(), pauline.getCMgr()}) {
			...
			if (!!join_after_termination) {
				...
				BC_ASSERT_PTR_NOT_NULL(participant_call);
				if (participant_call) {
					linphone_call_ref(participant_call);
				}
				...
				if (participant_call) {
					...
					linphone_call_unref(participant_call);
				}
			}
		}
...
void conference_joined_multiple_times_base(LinphoneConferenceSecurityLevel security_level,
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (int iteration = 0; iteration <= 4; iteration++) {
			...
			check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
			                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
			                         confAddr, security_level, encryption, enable_chat, FALSE, TRUE);
				...
				if (pconference) {
					...
					if (chat_room) {
						...
						if (chat_params) {
							...
							BC_ASSERT_EQUAL(linphone_chat_params_encryption_enabled(chat_params),
							                !!encrypted_conference, int, "%d");
						}
					}
					BC_ASSERT_TRUE(
			...
			if (expiry_after_s >= 0) {
				...
				if (fconference) {
					...
					for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
						LinphoneParticipantDevice *device = (LinphoneParticipantDevice *)bctbx_list_get_data(itd);
						time_t time_of_joining = linphone_participant_device_get_time_of_joining(device);
			...
			for (auto mgr : members) {
				...
				BC_ASSERT_PTR_NOT_NULL(call);
				if (call) {
					ms_message("Iteration[%0d]: %s is terminating call to %s", iteration,
					           linphone_core_get_identity(mgr->lc), conference_address_str);
					linphone_call_terminate(call);
				}
				...
				BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneCallReleased,
				                             ...
				                             liblinphone_tester_sip_timeout));
				BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneSubscriptionTerminated,
				                             mgr_stat.number_of_LinphoneSubscriptionTerminated + nb_client_subscribes,
				                             liblinphone_tester_sip_timeout));
				BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneConferenceStateTerminationPending,
			...
			// Wait a little bit
			ms_message("Iteration[%0d]: waiting that the conference information cleanup timer expires", iteration);
			CoreManagerAssert({focus, marie, pauline, laure, michelle, berthe})
			    .waitUntil(chrono::seconds(focus_cleanup_period), [] { return false; });
			...
			for (auto mgr : conferenceMgrs) {
				...
				if (!!linphone_core_conference_server_enabled(mgr->lc) && (focus_cleanup_period > 0) &&
					...
					if ((time_left > 0) && (time_left <= focus_cleanup_period)) {
						// Wait a little bit
						ms_message("Iteration[%0d]: waiting that the conference %s will be deleted the next time the "
						           "cleaup timer expires",
						           iteration, conference_address_str);
						CoreManagerAssert({focus, marie, pauline, laure, michelle, berthe})
						    .waitUntil(chrono::seconds(focus_cleanup_period + 1), [] { return false; });
					}

					now = ms_time(NULL);
					time_left = end_joining_window - now;
					if (time_left <= 0) {
						ms_message("Iteration[%0d]: conference information of conference %s was deleted on the server "
						           ...
						           iteration, conference_address_str, linphone_core_get_identity(mgr->lc), -time_left);
						info_deleted = true;
					}
				}
				LinphoneConferenceInfo *info = linphone_core_find_conference_information_from_uri(mgr->lc, confAddr);
...
void two_overlapping_conferences_base(bool_t same_organizer, bool_t dialout) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		std::list<LinphoneCoreManager *> members1{marie.getCMgr(), pauline.getCMgr(), laure.getCMgr()};
		...
		LinphoneParticipantRole role = LinphoneParticipantRoleSpeaker;
		for (auto &participant : participants1) {
			participantList1.insert(std::make_pair(
			    participant, add_participant_info_to_list(&participants_info1, participant->identity, role, -1)));
			role = (role == LinphoneParticipantRoleSpeaker) ? LinphoneParticipantRoleListener
			                                                : LinphoneParticipantRoleSpeaker;
		}
		...
		std::list<std::pair<LinphoneCoreManager *, stats>> member_stats_list1;
		for (auto mgr : members1) {
			member_stats_list1.push_back(std::make_pair(mgr, mgr->stat));
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members1,
		                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list1, marie.getCMgr(),
		                         confAddr1, security_level, LinphoneMediaEncryptionNone, FALSE, FALSE, TRUE);
		...
		for (auto &participant : participants2) {
			role = (role == LinphoneParticipantRoleSpeaker) ? LinphoneParticipantRoleListener
			                                                : LinphoneParticipantRoleSpeaker;
			participantList2.insert(std::make_pair(
			    participant, add_participant_info_to_list(&participants_info2, participant->identity, role, -1)));
		}

		std::list<LinphoneCoreManager *> members2{confCreator2.getCMgr()};
		std::list<std::pair<LinphoneCoreManager *, stats>> member_stats_list2{
		    std::make_pair(confCreator2.getCMgr(), confCreator2.getStats())};
		for (auto mgr : participants2) {
			members2.push_back(mgr);
			member_stats_list2.push_back(std::make_pair(mgr, mgr->stat));
		}
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members2,
		                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list2,
		                         confCreator2.getCMgr(), confAddr2, security_level, LinphoneMediaEncryptionNone, FALSE,
		                         dialout, TRUE);
	...
	{ // to make sure focus is destroyed after clients.
		...
		if (marie_participant) {
			...
			for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
				...
				BC_ASSERT_TRUE(linphone_participant_device_get_stream_capability(device, LinphoneStreamTypeVideo) ==
				               video_direction);
				BC_ASSERT_TRUE(linphone_participant_device_get_stream_availability(device, LinphoneStreamTypeVideo));
			}
			if (devices) {
				bctbx_list_free_with_data(devices, (bctbx_list_free_func)linphone_participant_device_unref);
			}
		...
		if (marie_participant) {
			...
			for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
				...
				BC_ASSERT_TRUE(linphone_participant_device_get_stream_capability(device, LinphoneStreamTypeVideo) ==
				               video_direction);
				BC_ASSERT_FALSE(linphone_participant_device_get_stream_availability(device, LinphoneStreamTypeVideo));
			}
			if (devices) {
				bctbx_list_free_with_data(devices, (bctbx_list_free_func)linphone_participant_device_unref);
			}
...
<p1:list>\n\
  ...
  <p1:entry uri=\"sip:pauline@sip.example.org\"/>\n\
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, LinphoneMediaEncryptionNone, FALSE, dialout, send_ics);
			        ...
			        .waitUntil(chrono::seconds(50), [mgr, &focus, &members, confAddr, &participantList] {
				        ...
				        if (conference) {
					        ...
					        for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
						        LinphoneParticipantDevice *device =
						            (LinphoneParticipantDevice *)bctbx_list_get_data(itd);
						        video_check &= !linphone_participant_device_get_stream_availability(
						            device, LinphoneStreamTypeVideo);
					        }
				        ...
				        for (const auto &[memberMgr, info] : participantList) {
					        LinphoneParticipant *participant =
					            linphone_conference_is_me(conference, memberMgr->identity)
					                ? linphone_conference_get_me(conference)
					                : linphone_conference_find_participant(conference, memberMgr->identity);
					        role_check &= (participant != nullptr);
					        if (participant) {
						        role_check &= (linphone_participant_get_role(participant) ==
						                       linphone_participant_info_get_role(info));
					        }
				        }
				        return role_check;
			        }));
...
void create_simple_conference_merging_calls_base(bool_t enable_ice,
	...
	{ // to make sure focus is destroyed after clients.
		...
		auto focus_stat = focus.getStats();
		std::list<std::pair<LinphoneCoreManager *, stats>> member_stats_list;
		for (auto mgr : participants) {
			member_stats_list.push_back(std::make_pair(mgr, mgr->stat));
		}
		...
		check_call_establishment({focus, marie, pauline, laure}, members, std::make_pair(focus.getCMgr(), focus_stat),
		                         member_stats_list, marie.getCMgr(), confAddr, security_level, encryption, enable_chat,
		                         FALSE, FALSE);
		int counter = 0;
		for (auto mgr : participants) {
			counter++;
		...
		if (enable_screen_sharing) {
			...
			if (marie_call) {
				...
				linphone_call_params_enable_screen_sharing(new_params, TRUE);
				toggle_screen_sharing({focus, marie, pauline, laure}, focus.getCMgr(), members, marie.getCMgr(),
				                      new_params, confAddr, TRUE);
				linphone_call_params_unref(new_params);
			}
		}
				...
				for (auto mgr : mgrList) {
					...
					if (pconference) {
						...
						for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
							...
							LinphoneMediaDirection video_dir =
							    linphone_participant_device_get_stream_capability(device, LinphoneStreamTypeVideo);
							if (linphone_conference_is_me(pconference,
							                              linphone_participant_device_get_address(device))) {
						...
						if (change_layout) {
							...
							LinphoneCall *participant_call2 = linphone_conference_get_call(pconference);
							BC_ASSERT_PTR_NOT_NULL(participant_call2);
							if (participant_call2) {
								const LinphoneCallParams *participant_call2_local_params =
								    linphone_call_get_params(participant_call2);
								const LinphoneConferenceLayout conference_layout =
								    linphone_call_params_get_conference_video_layout(participant_call2_local_params);
								...
								LinphoneCallParams *call_params =
								    linphone_core_create_call_params(mgr->lc, participant_call2);
								linphone_call_params_set_conference_video_layout(call_params, new_layout);
								linphone_call_update(participant_call2, call_params);
								linphone_call_params_unref(call_params);
							}
							...
							if (participant) {
								...
								for (bctbx_list_t *it_d = devices; it_d != NULL; it_d = it_d->next) {
									...
									LinphoneCall *participant_call = linphone_core_get_call_by_remote_address2(
									    focus.getLc(), linphone_participant_device_get_address(device));
									BC_ASSERT_PTR_NOT_NULL(participant_call);
									if (participant_call) {
							...
							if (participant_call) {
								const LinphoneCallParams *call_lparams = linphone_call_get_params(participant_call);
								BC_ASSERT_TRUE(linphone_call_params_video_enabled(call_lparams));
								const LinphoneCallParams *call_rparams =
								    linphone_call_get_remote_params(participant_call);
								BC_ASSERT_TRUE(linphone_call_params_video_enabled(call_rparams));
								const LinphoneCallParams *call_cparams =
								    linphone_call_get_current_params(participant_call);
							...
							LinphoneAddress *uri = mgr->identity;
							LinphoneCall *focus_call = linphone_core_get_call_by_remote_address2(focus.getLc(), uri);
							BC_ASSERT_PTR_NOT_NULL(focus_call);
							if (focus_call) {
								const LinphoneCallParams *call_lparams = linphone_call_get_params(focus_call);
								BC_ASSERT_TRUE(linphone_call_params_video_enabled(call_lparams));
	...
	{ // to make sure focus is destroyed after clients.
		...
		if (participant_codec_mismatch) {
			...
			for (const auto &m : codec_mismatch_members) {
				...
				auto itMemberStats = std::find_if(member_stats_list.begin(), member_stats_list.end(),
				                                  [&m](const auto &member_stat) { return member_stat.first == m; });
				if (itMemberStats != member_stats_list.end()) {
					member_stats_list.erase(itMemberStats);
				}
			...
			for (auto mgr : participants) {
				...
				if (participant_call) {
					...
					if (accept) {
						...
						linphone_call_decline(participant_call, LinphoneReasonDeclined);
					}
				}
			}
		}
		...
		if (accept) {
			...
			check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, members,
			                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
			                         confAddr, security_level, LinphoneMediaEncryptionNone, enable_chat, TRUE,
			                         send_ics);
			...
			for (auto mgr : conferenceMgrs) {
				...
				if (pconference) {
					...
					if (mgr == focus.getCMgr()) {
						...
						BC_ASSERT_PTR_NOT_NULL(focus_call);
						if (focus_call) {
							_linphone_call_check_max_nb_streams(focus_call, no_max_streams_audio, no_max_streams_video,
							                                    no_streams_text);
							_linphone_call_check_nb_active_streams(focus_call, no_streams_audio, no_streams_video,
							                                       no_streams_text);
							const LinphoneCallParams *call_lparams = linphone_call_get_params(focus_call);
						...
						for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
							...
							if (enable) {
								if (linphone_conference_is_me(paulineConference,
									...
									BC_ASSERT_TRUE(
									    linphone_participant_device_get_stream_availability(device,
									                                                        LinphoneStreamTypeVideo) ==
									    (linphone_participant_device_get_stream_capability(
									         device, LinphoneStreamTypeVideo) == LinphoneMediaDirectionSendRecv));
								}
							} else {
								BC_ASSERT_TRUE(linphone_participant_device_get_stream_availability(
								                   device, LinphoneStreamTypeVideo) == enable);
							}
						}
...
void create_conference_with_audio_only_participants_base(LinphoneConferenceSecurityLevel security_level) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		check_call_establishment({focus, marie, pauline, laure, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, encryption, FALSE, FALSE, TRUE);
		...
		for (auto mgr : conferenceMgrs) {
			...
			if (pconference) {
				...
				if (mgr == focus.getCMgr()) {
					...
					if (participant_call) {
						no_streams_audio = compute_no_audio_streams(participant_call, pconference);
						_linphone_call_check_nb_streams(participant_call, no_streams_audio, no_streams_video,
						                                no_streams_text);
						_linphone_call_check_nb_active_streams(participant_call, no_streams_audio,
						                                       no_active_streams_video, no_streams_text);
						const LinphoneCallParams *call_lparams = linphone_call_get_params(participant_call);
					...
					BC_ASSERT_PTR_NOT_NULL(focus_call);
					if (focus_call) {
						_linphone_call_check_nb_streams(focus_call, no_streams_audio, no_streams_video,
						                                no_streams_text);
						_linphone_call_check_nb_active_streams(focus_call, no_streams_audio, no_active_streams_video,
						                                       no_streams_text);
						const LinphoneCallParams *call_lparams = linphone_call_get_params(focus_call);
		...
		check_call_establishment({focus, marie, pauline, laure, berthe}, members,
		                         std::make_pair(focus.getCMgr(), focus_stat), new_member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, encryption, FALSE, FALSE, TRUE);
		...
		wait_for_conference_streams({focus, marie, pauline, laure, berthe}, conferenceMgrs, focus.getCMgr(),
		                            memberList2, confAddr, TRUE, security_level);
...
void create_simple_conference_dial_out_with_some_calls_declined_base(LinphoneReason reason,
	...
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		bool_t encrypted_conference = (securityLevel == LinphoneConferenceSecurityLevelEndToEnd ? TRUE : FALSE);
		const LinphoneTesterLimeAlgo lime_algo = encrypted_conference ? C25519 : UNSET;
		...
		std::list<std::pair<LinphoneCoreManager *, stats>> active_member_stats_list;
		for (auto mgr : all_active_participants) {
			active_member_stats_list.push_back(std::make_pair(mgr, mgr->stat));
		}
			...
			for (auto mgr : participants) {
				...
				if (participant_call) {
					...
					if (std::find(active_participants.cbegin(), active_participants.cend(), mgr) !=
						...
						linphone_call_decline(participant_call, reason);
					}
				}
		...
		check_call_establishment({focus, marie, pauline, laure, michelle, berthe}, all_active_participants,
		                         std::make_pair(focus.getCMgr(), focus_stat), active_member_stats_list, marie.getCMgr(),
		                         confAddr, security_level, LinphoneMediaEncryptionNone, FALSE, TRUE, FALSE);
		...
		for (auto mgr : conference_members) {
			...
			if (pconference) {
				...
				bctbx_list_free_with_data(participants_list, (bctbx_list_free_func)linphone_participant_unref);
		...
		for (int i = 0; i < 4; i++) {
			...
			if (paulineConference) {
				...
				for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
					...
					CoreManagerAssert({focus, marie, pauline, laure, michelle, berthe})
					    .waitUntil(chrono::seconds(10), [enable, paulineConference, device] {
...
void change_active_speaker_base(bool transfer_mode) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> participantList;
		LinphoneParticipantRole role = LinphoneParticipantRoleSpeaker;
		for (auto &participant : invitesList) {
			participantList.insert(std::make_pair(
			    participant, add_participant_info_to_list(&participants_info, participant->identity, role, -1)));
		}
		LinphoneAddress *confAddr =
		...
		for (auto mgr : members) {
			...
			check_call_establishment({focus, marie, pauline, laure}, active_members,
			                         ...
			                         marie.getCMgr(), confAddr, LinphoneConferenceSecurityLevelNone,
			                         LinphoneMediaEncryptionNone, FALSE, FALSE, TRUE);
		}
		...
		for (LinphoneCoreManager *mgr : conferenceMgrs) {
			...
			if (pconference) {
				...
				if (mgr == focus.getCMgr()) {
					...
					BC_ASSERT_PTR_NOT_NULL(participant_call);
					if (participant_call) {
						_linphone_call_check_max_nb_streams(participant_call, nbStreamsAudio,
						                                    (mgr != pauline.getCMgr() ? nbStreamsVideo : 0),
						                                    nbStreamsText);
					}
					LinphoneCall *focus_call = linphone_core_get_call_by_remote_address2(focus.getLc(), mgr->identity);
					BC_ASSERT_PTR_NOT_NULL(focus_call);
					if (focus_call) {
						_linphone_call_check_max_nb_streams(
						    focus_call, nbStreamsAudio, (mgr != pauline.getCMgr() ? nbStreamsVideo : 0), nbStreamsText);
					}
				}
