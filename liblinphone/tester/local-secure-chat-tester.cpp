...
static void secure_group_chat_room_with_client_sending_messages_without_receiving_notify(void) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		BC_ASSERT_TRUE(CoreManagerAssert({focus, marie, michelle, pauline}).wait([&marie] {
			const auto &chatRooms = marie.getCore().getChatRooms();
			if (chatRooms.size() != 1) {
				return false;
			}
...
static void secure_one_on_one_chatroom_not_rejoined_after_leaving() {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		const auto lime_algo = C25519;
		ClientConference marie("marie_rc", focus.getConferenceFactoryAddress(), lime_algo);
