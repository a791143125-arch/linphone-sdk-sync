...
extern void text_message_base_with_text(LinphoneCoreManager *marie,
...
static void sync_message_call_logs(bool_t with_call_log_server) {
	...
	for (int testCaseIdx = MISSED_CASE; testCaseIdx <= ACCEPTED_CASE; ++testCaseIdx) {
		...
		if (!BC_ASSERT_TRUE(
		    wait_for_list(lcs, &offlineMarie->stat.number_of_LinphoneCoreCallLogUpdated, callLogUpdated + 1, 10000)))
			lWarning() << "number_of_LinphoneCoreCallLogUpdated=" << offlineMarie->stat.number_of_LinphoneCoreCallLogUpdated;
		...
		if (offlineMarieLogs){
			offlineCallLogRef->setDuration(CallLog::toCpp((LinphoneCallLog *)bctbx_list_get_data(offlineMarieLogs))->getDuration());
		}
		const bctbx_list_t *onlineMarieLogs = linphone_core_get_call_logs(onlineMarie->lc);
		if (onlineMarieLogs) {
			onlineCallLogRef->setDuration(CallLog::toCpp((LinphoneCallLog *)bctbx_list_get_data(onlineMarieLogs))->getDuration());
		}
