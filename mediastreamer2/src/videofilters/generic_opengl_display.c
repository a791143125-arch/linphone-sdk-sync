...
public:
	WindowIdTrace(){}
	WindowIdTrace(uint64_t filterAddress, uint64_t glDisplay, uint64_t windowId, const std::string &stackTrace, uint64_t t)
		: mFilterAddress(filterAddress), mGlDisplay(glDisplay),  mWindowId(windowId), mStackTrace(stackTrace), mTime(t)
	{
	}
	uint64_t mFilterAddress=0;
	uint64_t mWindowId=0;
	uint64_t mGlDisplay=0;
...
typedef struct _MSOGLSharedContext {
	MSWorkerThread *process_thread;
	int use_count;
	std::map<uint64_t, WindowIdTrace> windowId; // Key=Filter address
} MSOGLSharedContext;
...
static void msogl_update_window_id_duplicate(uint64_t filter, uint64_t id) {
	ms_mutex_lock(&shared_context_lock);
	if (id) { // Backup the stacktrace and the ID for this filter
		shared_context.windowId[filter] = WindowIdTrace(filter, display, id, bctoolbox::Utils::getStackTraceAsString(0), bctbx_get_cur_time_ms());
...
static bool msogl_check_window_id_duplicate() {
	...
	for (auto filterId : shared_context.windowId) {
		if(++ids[filterId.second.first] > 1) {
			haveDuplicate = true;
		}
	}
	if (haveDuplicate) { // No need to process if no duplication.
		for (auto nativeId : ids) {
			if (nativeId.second > 1) { // Get all filters with this ID and display stacktrace
				ms_warning("[MSOGL] %d duplicate native ID use detected for [0x%lX]", nativeId.second, nativeId.first);
				for (auto filterId : shared_context.windowId) {
					if (filterId.second.mWindowId == nativeId.first) {
						ms_warning("Stacktrace when the ID was set at %lu for the Filter [0x%lX] and Display [0x%lX]:\n%s",filterId.second.mTime,
							filterId.first, filterId.second.mGlDisplay,
							filterId.second.mStackTrace.c_str());
					}
				}
			}
		}
...
static void ogl_uninit(MSFilter *f) {
	...
	msogl_update_window_id_duplicate((uint64_t)f, (uint64_t)data->display, 0);
...
static int ogl_set_native_window_id(MSFilter *f, void *arg) {
	...
	msogl_update_window_id_duplicate((uint64_t)f, (uint64_t)data->display, (uint64_t)(context_info ? context_info->window : 0));
