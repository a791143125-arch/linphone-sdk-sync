	...
	if (ctx->mChannelStatus == DtlsStatus::HandshakeOngoing) {
		if (ret == 0) {                       /* handshake is over, give the keys to srtp : 128 bits client write - 128
			...
			if (ctx->mVerifyCertificate) {    // Certificate was verified, now check that it holds a subject (or cname)
				if (!ctx->mPeerUri.empty()) { // matching the caller name
					if (bctbx_x509_certificate_subject_match(
					        bctbx_ssl_get_peer_certificate(ctx->mDtlsCryptoContext.ssl), ctx->mPeerUri.c_str())) {
			...
			if (ctx->mSrtpProtectionProfile == MS_CRYPTO_SUITE_INVALID) {
				...
				if (ms_dtls_srtp_check_certificate_fingerprint(
				        bctbx_ssl_get_peer_certificate(ctx->mDtlsCryptoContext.ssl), ctx->mPeerFingerprint) == 1) {
					ctx->setKeyMaterial();
