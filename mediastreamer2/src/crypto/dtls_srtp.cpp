...
public:
	...
	~DtlsCrypto() {
		...
		bctbx_x509_certificate_free(root_ca);
...
struct _MSDtlsSrtpContext {
	...
	DtlsStatus mChannelStatus;     /**< channel status :not ready, ready, hanshake on going, handshake over, fingerprint
	                                  verified, failed */
	bool mVerifyCertificate;       /**< shall we verify the peer certificate? */
	std::string mPeerUri;          /**< the peer uri as announced in the SDP */
	...
	_MSDtlsSrtpContext(MSMediaStreamSessions *sessions, MSDtlsSrtpParams *params)
	    : mStreamSessions(sessions), mRole(params->role), mtu(params->mtu),
	      mVerifyCertificate(params->verify_certificate), mPeerUri(std::string(params->peer_uri)) {
	...
	bool isDtlsPacket(mblk_t *msg);
	bool isPeerCerticateMatchHisUri();
};
...
void schedule_rtp(struct _RtpTransportModifier *t) {
	...
	/* when the handshake failed : do not resend anything */
	if (ctx->mChannelStatus == DtlsStatus::HandshakeFailed) {
		return;
	}
...
int ms_dtls_srtp_rtp_process_on_receive(struct _RtpTransportModifier *t, mblk_t *msg) {
	MSDtlsSrtpContext *ctx = (MSDtlsSrtpContext *)t->data;
	if (!ctx->isDtlsPacket(msg)) {
		return (int)msgdsize(msg);
	}

	std::lock_guard<std::mutex> lock(ctx->mtx);
	if (ctx->mChannelStatus == DtlsStatus::HandshakeFailed) {
		ms_message("DTLS RTP received a message but we are in failed state: ignore");
		return 0;
	}
	...
	if (ret == 0 && ctx->mChannelStatus == DtlsStatus::HandshakeOngoing) {
		/* handshake is now over */
		if (ctx->mVerifyCertificate) { // Certificate was verified, now check that it holds a subject (or cname)
			if (!ctx->isPeerCerticateMatchHisUri()) {
				return 0;
			}
		}
		...
		if (ctx->mSrtpProtectionProfile == MS_CRYPTO_SUITE_INVALID) {
			...
			ms_message("DTLS SRTP Handshake on RTP channel successful, srtp protection profile %d",
			...
			if (ms_dtls_srtp_check_certificate_fingerprint(bctbx_ssl_get_peer_certificate(ctx->mDtlsCryptoContext.ssl),
				...
				ctx->setKeyMaterial();
				ctx->setChannelStatus(DtlsStatus::FingerprintVerified);
			}
		}
	}
	return 0;
...
int MSDtlsSrtpContext::initialiseDtlsCryptoContext(MSDtlsSrtpParams *params) {
	...
	std::error_code ec;
	std::filesystem::file_status s = std::filesystem::status(params->root_ca, ec);
	if (!ec) {
		int error = -1;
		if (std::filesystem::is_directory(s)) {
			error = bctbx_x509_certificate_parse_path(mDtlsCryptoContext.root_ca, params->root_ca);
		} else if (std::filesystem::is_regular_file(s)) {
			error = bctbx_x509_certificate_parse_file(mDtlsCryptoContext.root_ca, params->root_ca);
		}
		if (error < 0) {
			char errorstr[512];
			bctbx_strerror(error, errorstr, sizeof(errorstr));
			ms_error("DTLS failed to load root ca from %s: %s", params->root_ca, errorstr);
		}
	} else {
		ms_error("DTLS failed to load root ca from %s: %s", params->root_ca, ec.message().c_str());
	}
	...
	/* set certificates */
	/* Even if we do not verify the peer certificate, set it to verify optional, this will force server to send his
...
void MSDtlsSrtpContext::setChannelStatus(DtlsStatus status, MSDtlsError error) {
	...
	if ((status == DtlsStatus::FingerprintVerified) && (rtp_session_rtcp_mux_enabled(mStreamSessions->rtp_session))) {
...
/**
 * Check if we are ready to process a DTLS packet and if it is one
 * @param msg	incoming message
 * @return true if we have a DTLS packet and ready to process it
 */
bool MSDtlsSrtpContext::isDtlsPacket(mblk_t *msg) {
	/* check if we have an on-going handshake */
	if (mChannelStatus == DtlsStatus::ContextNotReady) {
		return false;
	}

	// check incoming message length
	if (msgdsize(msg) < RTP_FIXED_HEADER_SIZE) {
		return false;
	}
...
/**
 * Check if the peer's uri stored in context matches peer's certificate subject
 * @ return true if the handshake was successful and the peer's uri matches the subject
 */
bool MSDtlsSrtpContext::isPeerCerticateMatchHisUri() {
	if (!mPeerUri.empty()) { // matching the peer's name with the one from his certificate
		if (bctbx_x509_certificate_subject_match(bctbx_ssl_get_peer_certificate(mDtlsCryptoContext.ssl),
		                                         mPeerUri.c_str())) {
			ms_message("DTLS SRTP handshake successful with peer certificate verified, it holds a SAN or "
			           "CN matchings "
			           "the peer identity %s",
			           mPeerUri.c_str());
			return true;
		} else {
			ms_error("DTLS SRTP handshake successful but peer certificate does not hold a SAN or CN matchings "
			         "its sip:uri %s",
			         mPeerUri.c_str());
			bctbx_ssl_send_fatal_alert_message(mDtlsCryptoContext.ssl, BCTBX_TLS_ALERT_ACCESS_DENIED);
			setChannelStatus(DtlsStatus::HandshakeFailed, MS_DTLS_ERROR_CERT_SUBJECT_UNMATCHING);
			return false;
		}
	} else { // no peer uri to match it
